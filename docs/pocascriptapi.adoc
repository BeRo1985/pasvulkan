...
The Sprite type represents a 2D image or sprite atlas region that can be rendered on screen. Sprite objects are typically created and managed by the host application and passed to scripts.

'''''

=== Sprite Creation

Sprites are not directly created in scripts - they are provided by the host application through the game/application logic. Sprites can be drawn using Canvas methods like `+drawSprite()+`.

*Example:*

[source,js]
----
// Sprites are provided by the host application
// Then used in drawing operations
canvas.drawSprite(mySprite, x, y, width, height);
----
...
The SpriteAtlas type manages a collection of sprites packed into a single texture atlas for efficient rendering. SpriteAtlas objects are created and managed by the host application.

'''''

=== SpriteAtlas Usage

SpriteAtlas objects are provided by the host application. Individual sprites within an atlas can be accessed and rendered through the host interface.

*Example:*

[source,js]
----
// SpriteAtlas is managed by the host application
// Access individual sprites and render them
canvas.drawSprite(atlas.getSprite("character"), x, y, width, height);
----
...
The Font type represents a font that can be used for text rendering. Font objects are typically loaded and managed by the host application.

'''''

=== Font Usage

Font objects are provided by the host application and can be used with Canvas text drawing methods through the Canvas font settings.

*Example:*

[source,js]
----
// Fonts are typically provided by the host application
canvas.setFont(myFont);
canvas.setFontSize(24);
canvas.drawText("Hello World", x, y);
----
