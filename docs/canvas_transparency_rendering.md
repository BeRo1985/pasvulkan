...
-   **Shape ID and Reset**: The 24-bit ID (~16.7 million) is sufficient for most scenes. When approaching the limit, **clearing the entire coverage buffer** is necessary. This overhead can be amortized by **batch processing shapes** (i.e., accumulating a large number of shapes until a reset is needed).
