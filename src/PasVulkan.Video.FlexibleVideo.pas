 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Video.FlexibleVideo;
{$i PasVulkan.inc}
{$ifndef fpc}
...
// "fwv" tools / the "FVDC" container. This base unit holds only the parts the decoder and the encoder have
...
// per-frame payload framing. PasVulkan.Video.FlexibleVideo.Decoder and .Encoder build the Vulkan-compute
// codec classes on top of it; the motion-vector entropy coders are added here as the inter-frame stages
// need them.
//
// The arithmetic is kept bit-for-bit identical to the C reference. The .fvd frame payloads use one of three
...
type EpvFlexibleVideo=class(Exception);

     { TpvFlexibleVideo }
     TpvFlexibleVideo=class // plain TObject base, resource-system-independent (NOT TpvResource)
      public
       const Magic:array[0..3] of AnsiChar=('F','V','D','C'); // the .fvd container magic
             FormatVersion=1;
      ...
      public
       // ---- DCT (FVD) decode support: rANS GPU table, JPEG DCT quant matrix, adaptive quad-tree partitions ----
       const DCTBlockSize=8;                  // 8x8 block DCT
             QuadTreeRegion=32;               // quad-tree root region (32 -> 16 -> 8)
             QuadTreeMaximumLeaves=16;        // worst-case leaves per 32x32 region
             RANSDCSymbols=17;
             RANSACSymbols=256;
             RANSTotalFrequency=4096;         // sum of normalized frequencies (1 shl 12)
             // GPU rANS table uint32 layout: [DCNormalized 17][DCCumulative 17][ACNormalized 256][ACCumulative 256][DCSlot 4096][ACSlot 4096]
             RANSGPUOffsetDCNormalized=0;
             RANSGPUOffsetDCCumulative=RANSDCSymbols;                          // 17
             RANSGPUOffsetACNormalized=RANSDCSymbols*2;                        // 34
             RANSGPUOffsetACCumulative=RANSGPUOffsetACNormalized+RANSACSymbols;// 290
             RANSGPUOffsetDCSlot=RANSGPUOffsetACCumulative+RANSACSymbols;      // 546
             RANSGPUOffsetACSlot=RANSGPUOffsetDCSlot+RANSTotalFrequency;       // 4642
             RANSGPUTableUInt32Count=RANSGPUOffsetACSlot+RANSTotalFrequency;   // 8738 (*4 = 34952 bytes per plane)
      public
       // Build the per-plane GPU rANS frequency table (normalized + exclusive-cumulative + slot tables) from the appended
       // entropy blob (17 DC + 256 AC exp-golomb normalized frequencies). Mirror of the C rans_build_gpu_table; aOutputTable has RANSGPUTableUInt32Count uint32s.
       class procedure BuildRANSGPUTable(const aBlob:PpvUInt8Array;const aBlobLength:TpvUInt32;const aOutputTable:PpvUInt32Array); static;
       // JPEG-style DCT quant step matrix (8x8 base, replicated periodically) into a per-pixel step buffer; mirror of
       // build_dct_quant_matrix_table. aChroma picks the chroma table. Height is padded to /8 internally (partial bottom block).
...
{ TpvFlexibleVideo.TBitReader }

procedure TpvFlexibleVideo.TBitReader.Init(const aBytes:PpvUInt8Array;const aByteLength:TpvSizeUInt);
begin
 Bytes:=aBytes;
...
procedure TpvFlexibleVideo.TBitReader.Refill;
begin
...
function TpvFlexibleVideo.TBitReader.GetBit:TpvInt32;
begin
...
function TpvFlexibleVideo.TBitReader.GetBits(const aBitCount:TpvInt32):TpvUInt32;
begin
...
function TpvFlexibleVideo.TBitReader.GetUnsignedExpGolomb:TpvUInt32;
...
function TpvFlexibleVideo.TBitReader.GetSignedExpGolomb:TpvInt32;
...
class function TpvFlexibleVideo.ReadU32LE(const aBytes:PpvUInt8Array;const aOffset:TpvSizeUInt):TpvUInt32;
begin
...
class function TpvFlexibleVideo.LZSSDecompress(const aInput:PpvUInt8Array;const aInputLength:TpvSizeUInt;const aOutput:PpvUInt8Array;const aOutputSize:TpvSizeUInt):boolean;
...
class function TpvFlexibleVideo.DecompressFrame(const aCompressed:PpvUInt8Array;const aCompressedLength:TpvSizeUInt;const aOutput:PpvUInt8Array;const aOutputCapacity:TpvSizeUInt;out aRawLength:TpvUInt32):boolean;
var Method:TpvUInt8;
    DestData:TpvPointer;
...
class function TpvFlexibleVideo.ParseFrameHeader(const aFrame:PpvUInt8Array;const aFrameLength:TpvSizeUInt;const aBlockCount:TBlockCounts;const aOffsets:TPlaneOffsets;out aLeadingBlockCount:TpvInt32;out aMVDataOffset:TpvSizeUInt;out aMVLength:TpvUInt32;out aBlockDataOffset:TpvSizeUInt):boolean;
...
class procedure TpvFlexibleVideo.MeasureSynthesisGains(const aLevels:TpvInt32;out aHFGain:TSynthesisGains;out aLLGain:TpvFloat);
...
class procedure TpvFlexibleVideo.BuildQuantizationSteps(const aStep:PpvInt32Array;const aWidth,aHeight,aLevels,aBaseQuality,aSampleWhite:TpvInt32;const aHFGain:TSynthesisGains;const aLLGain:TpvFloat);
...
{ TpvFlexibleVideo — AQ (per-tile QP / adaptive quantization) }

class function TpvFlexibleVideo.AQTileCols(const aWidth:TpvInt32):TpvInt32;
begin
 result:=(aWidth+(AQTile-1)) div AQTile;
end;

class function TpvFlexibleVideo.AQTileRows(const aHeight:TpvInt32):TpvInt32;
begin
...
class procedure TpvFlexibleVideo.ApplyTileAQ(const aStep:PpvInt32Array;const aWidth,aHeight,aLevels:TpvInt32;const aTileCode:PpvUInt8Array;const aTileCols,aTileRows:TpvInt32);
...
class procedure TpvFlexibleVideo.DecodeMotionVectors(var aReader:TBitReader;const aMV:PpvInt32Array;const aBlocksX,aBlocksY:TpvInt32);
...
function FWVReadFlagRLE(var aReader:TpvFlexibleVideo.TBitReader;const aFlags:PpvUInt8Array):TpvInt32;
...
procedure FWVDecodeQuadtreeMVs(var aReader:TpvFlexibleVideo.TBitReader;const aFineMV:PpvInt32Array;const aFineGridX,aFineGridY,aX0,aY0,aCells:TpvInt32;const aFlags:PpvUInt8Array;var aFlagIndex:TpvInt32);
...
class procedure TpvFlexibleVideo.DecodeMotionQuadtree(var aReader:TBitReader;const aFineMV:PpvInt32Array;const aFineGridX,aFineGridY:TpvInt32);
...
{ TpvFlexibleVideo.TMVRangeDecoder }

procedure TpvFlexibleVideo.TMVRangeDecoder.Init(const aInput:PpvUInt8Array;const aByteLength:TpvSizeUInt);
...
function TpvFlexibleVideo.TMVRangeDecoder.DecodeBit(var aProbability:TpvUInt16):TpvInt32;
var Bound:TpvUInt32;
begin
...
function TpvFlexibleVideo.TMVRangeDecoder.DecodeBypass:TpvInt32;
var Bound:TpvUInt32;
begin
 Bound:=Range shr 1;
 if Code<Bound then begin
...
function TpvFlexibleVideo.TMVRangeDecoder.DecodeResidual(const aComponent,aBucket:TpvInt32):TpvInt32;
...
function TpvFlexibleVideo.TMVRangeDecoder.DecodeMode:TpvInt32;
begin
...
class procedure TpvFlexibleVideo.DecodeMotionVectorsRange(var aDecoder:TMVRangeDecoder;const aMV:PpvInt32Array;const aBlocksX,aBlocksY:TpvInt32);
...
procedure FWVDecodeQuadtreeMVsRange(var aDecoder:TpvFlexibleVideo.TMVRangeDecoder;const aFineMV:PpvInt32Array;const aFineGridX,aFineGridY,aX0,aY0,aCells:TpvInt32);
var Split,Component,Prediction,Half,dx,dy,Index:TpvInt32;
    MV:array[0..1] of TpvInt32;
begin
 if aCells<>1 then begin
...
class procedure TpvFlexibleVideo.DecodeMotionQuadtreeRange(var aDecoder:TMVRangeDecoder;const aFineMV:PpvInt32Array;const aFineGridX,aFineGridY:TpvInt32);
...
procedure FWVDecodeModeLeaves(var aReader:TpvFlexibleVideo.TBitReader;const aMode:PpvInt32Array;const aFineGridX,aFineGridY,aX0,aY0,aCells:TpvInt32;const aFlags:PpvUInt8Array;var aFlagIndex:TpvInt32);
...
class procedure TpvFlexibleVideo.DecodeModeQuadtree(var aReader:TBitReader;const aMode:PpvInt32Array;const aFineGridX,aFineGridY:TpvInt32);
var RootCells,Grid32X,Grid32Y,Capacity,FlagIndex,RootX,RootY:TpvInt32;
    Flags:array of TpvUInt8;
begin
 RootCells:=MotionRoot div MotionLeaf;
...
procedure FWVDecodeModeLeavesRange(var aDecoder:TpvFlexibleVideo.TMVRangeDecoder;const aMode:PpvInt32Array;const aFineGridX,aFineGridY,aX0,aY0,aCells:TpvInt32);
var Split,Mode,Half,dx,dy:TpvInt32;
begin
 if aCells<>1 then begin
  Split:=aDecoder.DecodeBit(aDecoder.Flag);
...
class procedure TpvFlexibleVideo.DecodeModeQuadtreeRange(var aDecoder:TMVRangeDecoder;const aMode:PpvInt32Array;const aFineGridX,aFineGridY:TpvInt32);
var RootCells,RootX,RootY:TpvInt32;
begin
 RootCells:=MotionRoot div MotionLeaf;
 RootY:=0;
...
class function TpvFlexibleVideo.TemporalQuantLevel(const aFrame,aGOPCount,aTemporalLevels:TpvInt32):TpvInt32;
...
class function TpvFlexibleVideo.TemporalQuantScale(const aLevel:TpvInt32):TpvFloat;
const Beta=0.6;
      Kappa=1.14;
begin
...
{ TpvFlexibleVideo — DCT (FVD) decode support }

class function TpvFlexibleVideo.QuadTreeRegionCount(const aExtent:TpvInt32):TpvInt32;
begin
 result:=(aExtent+(QuadTreeRegion-1)) div QuadTreeRegion;
end;

class procedure TpvFlexibleVideo.BuildRANSGPUTable(const aBlob:PpvUInt8Array;const aBlobLength:TpvUInt32;const aOutputTable:PpvUInt32Array);
var Reader:TBitReader;
    SymbolIndex,SlotIndex:TpvInt32; // signed: a symbol's normalized frequency can be 0 (an unsigned `0 to norm-1` would underflow)
    SlotCursor,RunningCumulative:TpvUInt32;
    DCNormalized:array[0..RANSDCSymbols-1] of TpvUInt32;
    ACNormalized:array[0..RANSACSymbols-1] of TpvUInt32;
begin
 Reader.Init(aBlob,aBlobLength);
 for SymbolIndex:=0 to RANSDCSymbols-1 do begin
  DCNormalized[SymbolIndex]:=Reader.GetUnsignedExpGolomb;
 end;
 for SymbolIndex:=0 to RANSACSymbols-1 do begin
  ACNormalized[SymbolIndex]:=Reader.GetUnsignedExpGolomb;
 end;
 // normalized + exclusive-prefix cumulative frequencies
 RunningCumulative:=0;
 for SymbolIndex:=0 to RANSDCSymbols-1 do begin
  aOutputTable^[RANSGPUOffsetDCNormalized+SymbolIndex]:=DCNormalized[SymbolIndex];
  aOutputTable^[RANSGPUOffsetDCCumulative+SymbolIndex]:=RunningCumulative;
  inc(RunningCumulative,DCNormalized[SymbolIndex]);
 end;
 RunningCumulative:=0;
 for SymbolIndex:=0 to RANSACSymbols-1 do begin
  aOutputTable^[RANSGPUOffsetACNormalized+SymbolIndex]:=ACNormalized[SymbolIndex];
  aOutputTable^[RANSGPUOffsetACCumulative+SymbolIndex]:=RunningCumulative;
  inc(RunningCumulative,ACNormalized[SymbolIndex]);
 end;
 // slot tables: one symbol id per of the RANSTotalFrequency slots
 SlotCursor:=0;
 for SymbolIndex:=0 to RANSDCSymbols-1 do begin
  for SlotIndex:=0 to TpvInt32(DCNormalized[SymbolIndex])-1 do begin
   aOutputTable^[RANSGPUOffsetDCSlot+SlotCursor+TpvUInt32(SlotIndex)]:=TpvUInt32(SymbolIndex);
  end;
  inc(SlotCursor,DCNormalized[SymbolIndex]);
 end;
 SlotCursor:=0;
 for SymbolIndex:=0 to RANSACSymbols-1 do begin
  for SlotIndex:=0 to TpvInt32(ACNormalized[SymbolIndex])-1 do begin
   aOutputTable^[RANSGPUOffsetACSlot+SlotCursor+TpvUInt32(SlotIndex)]:=TpvUInt32(SymbolIndex);
  end;
  inc(SlotCursor,ACNormalized[SymbolIndex]);
 end;
end;
...
const JPEGLuminanceTable:array[0..63] of TpvInt32=(
      ...
      JPEGChrominanceTable:array[0..63] of TpvInt32=(
...
begin
 ...
 // Scale the quality to the reference white (SDR x1, HDR-12 x16) so a given quality means the same coarseness at any bit depth.
 ScaledQuality:=aBaseQuality*(aSampleWhite div 256);
 // Pad the height to a multiple of 8: the partial bottom block (e.g. 4:2:0 chroma 540) still gets dequantized. The
 // 8x8 base matrix is periodic in the row (BlockRow = PixelY mod 8), so the padded rows are well-defined; the step buffer is full-size.
 PaddedHeight:=((aHeight+(DCTBlockSize-1)) div DCTBlockSize)*DCTBlockSize;
 for PixelY:=0 to PaddedHeight-1 do begin
  BlockRow:=PixelY and 7;
  for PixelX:=0 to aWidth-1 do begin
   BlockColumn:=PixelX and 7;
   QuantStep:=Trunc((((ScaledQuality*QuantTable^[(BlockRow*8)+BlockColumn])/16.0))+0.5);
   if QuantStep<1 then begin
    QuantStep:=1;
   end;
