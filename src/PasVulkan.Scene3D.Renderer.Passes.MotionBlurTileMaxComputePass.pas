 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.MotionBlurTileMaxComputePass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesMotionBlurTileMaxComputePass }
     TpvScene3DRendererPassesMotionBlurTileMaxComputePass=class(TpvFrameGraph.TComputePass)
      public
...
{ TpvScene3DRendererPassesMotionBlurTileMaxComputePass  }

constructor TpvScene3DRendererPassesMotionBlurTileMaxComputePass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
 inherited Create(aFrameGraph);

 fInstance:=aInstance;

 Name:='MotionBlurTileMaxComputePass';

 begin

  begin

   fResourceInput:=AddImageInput('resourcetype_velocity',
                                 'resource_velocity_data',
                                 VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                 []
                                );

   fResourceOutput:=AddImageOutput('resourcetype_motionblur_tile',
                                   'resource_motionblur_tilemax',
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
...
destructor TpvScene3DRendererPassesMotionBlurTileMaxComputePass.Destroy;
begin
 inherited Destroy;
end;

procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.AcquirePersistentResources;
var Stream:TStream;
begin

 inherited AcquirePersistentResources;

 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('motionblur_tilemax_comp.spv');
 try
...
procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.AcquireVolatileResources;
...
begin
 ...
 fPipelineLayout.AddPushConstantRange(TVkShaderStageFlags(VK_SHADER_STAGE_COMPUTE_BIT),0,SizeOf(TpvScene3DRendererPassesMotionBlurTileMaxComputePass.TPushConstants));
 ...
 // Always an array view, even for a single view. The shaders here are written against sampler2DArray and
 // image2DArray throughout, so that one piece of code serves the single and the multi-view case; handing
 // them a plain 2D view for one view is a type mismatch the validation layers do not catch and the
 // hardware answers with black.
...
procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.ReleaseVolatileResources;
...
procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesMotionBlurTileMaxComputePass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;const aInFlightFrameIndex,aFrameIndex:TpvSizeInt);
var InFlightFrameIndex,CountViews:TpvInt32;
    PushConstants:TpvScene3DRendererPassesMotionBlurTileMaxComputePass.TPushConstants;
...
begin
 ...
 // image may NOT be written in. So it is taken to GENERAL here and given back afterwards - and that
 // second barrier is also what makes the write visible to the pass that reads the tiles next.
 ...
 begin
  ...
  PushConstants.TileSize:=TpvScene3DRendererInstance.MotionBlurTileSize;

  aCommandBuffer.CmdPushConstants(fPipelineLayout.Handle,
                                  TVkShaderStageFlags(TVkShaderStageFlagBits.VK_SHADER_STAGE_COMPUTE_BIT),
                                  0,
                                  SizeOf(TpvScene3DRendererPassesMotionBlurTileMaxComputePass.TPushConstants),
                                  @PushConstants);
  ...
  if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
   fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Dispatch,'MotionBlurTileMax');
  end;
  // One workgroup per tile, and the tile buffer is one texel per tile, so its size IS the dispatch.
  aCommandBuffer.CmdDispatch(Max(1,fResourceOutput.Width),
                             Max(1,fResourceOutput.Height),
                             CountViews);
  if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
