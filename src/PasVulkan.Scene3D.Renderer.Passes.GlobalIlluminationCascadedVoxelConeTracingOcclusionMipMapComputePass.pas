...
begin
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('global_illumination_voxel_occlusion_mipmap_comp.spv');
 try
 ...
 for MipMapIndex:=1 to fInstance.GlobalIlluminationCascadedVoxelConeTracingOcclusionImages[0].MipMapLevels-1 do begin
  ...
  for CascadeIndex:=0 to fInstance.Renderer.GlobalIlluminationVoxelCountCascades-1 do begin
   ...
   if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
    fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Dispatch,'GIVoxelOcclusionMipMap');
   end;
