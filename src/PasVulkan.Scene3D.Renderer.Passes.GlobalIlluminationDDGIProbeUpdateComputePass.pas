...
begin
 ...
 for InFlightFrameIndex:=0 to fInstance.Renderer.CountInFlightFrames-1 do begin
  ...
  fVulkanDescriptorSets[InFlightFrameIndex].WriteToDescriptorSet(0,0,1,TVkDescriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER),[],[fInstance.GlobalIlluminationDDGIMasterBuffers[InFlightFrameIndex].DescriptorBufferInfo],[],false); // dormant pass (never created) — repointed only so it still compiles after the ddgiData merge
...
begin
 ...
 PushConstants.Master:=fInstance.GlobalIlluminationDDGIMasterBuffers[aInFlightFrameIndex].DeviceAddress; // BDA pointer to the ray-data sub-buffer (shared by irradiance/visibility/relocation/classification)
