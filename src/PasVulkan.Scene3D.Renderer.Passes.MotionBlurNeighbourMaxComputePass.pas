 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.MotionBlurNeighbourMaxComputePass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass }
     TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass=class(TpvFrameGraph.TComputePass)
      public
...
{ TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass  }

constructor TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
 inherited Create(aFrameGraph);

 fInstance:=aInstance;

 Name:='MotionBlurNeighbourMaxComputePass';

 begin

  begin

   fResourceInput:=AddImageInput('resourcetype_motionblur_tile',
...
destructor TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.Destroy;
begin
 inherited Destroy;
end;

procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.AcquirePersistentResources;
var Stream:TStream;
begin

 inherited AcquirePersistentResources;

 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('motionblur_neighbourmax_comp.spv');
 try
...
procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.AcquireVolatileResources;
...
begin
 ...
 fPipelineLayout.AddPushConstantRange(TVkShaderStageFlags(VK_SHADER_STAGE_COMPUTE_BIT),0,SizeOf(TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.TPushConstants));
...
procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.ReleaseVolatileResources;
...
procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;const aInFlightFrameIndex,aFrameIndex:TpvSizeInt);
var InFlightFrameIndex,CountViews:TpvInt32;
    PushConstants:TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.TPushConstants;
...
begin
 ...
 // second barrier is also what makes the write visible to the filter that reads it next.
 ...
 begin
  ...
  // The shader has no use for it - the neighbourhood is three by three tiles whatever a tile is - but the
  // range stays declared so the two tile passes keep one pipeline layout between them.
  PushConstants.TileSize:=TpvScene3DRendererInstance.MotionBlurTileSize;

  aCommandBuffer.CmdPushConstants(fPipelineLayout.Handle,
                                  TVkShaderStageFlags(TVkShaderStageFlagBits.VK_SHADER_STAGE_COMPUTE_BIT),
                                  0,
                                  SizeOf(TpvScene3DRendererPassesMotionBlurNeighbourMaxComputePass.TPushConstants),
                                  @PushConstants);
  ...
  if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
   fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Dispatch,'MotionBlurNeighbourMax');
  end;
  // One invocation per tile here, not one workgroup: the nine reads are cheap enough that there is
  // nothing to reduce and nothing to share.
