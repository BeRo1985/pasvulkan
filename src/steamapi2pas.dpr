...
const UnitName='PasVulkan.Steamworks';
      ...
      // The two records whose readable field layout lives in a separate view record, because their
      // declared type has to be byte array storage to keep an alignment of one. The layout verifier
      // has to look at the view rather than at the storage.
      ViewedStructNames:array[0..1] of TText=
       (
        'SteamNetworkingIPAddr',
        'SteamNetworkingIdentity'
       );

      // Records that the public headers only forward declare, since their definition lives in the
      // separate Steam Datagram Relay SDK. The layout verifier has to skip them, a C++ sizeof of an
      // incomplete type does not compile. Both hold nothing but int and char arrays, so no packing
      // regime can shift them anyway.
      IncompleteStructNames:array[0..1] of TText=
       (
        'SteamDatagramGameCoordinatorServerLogin',
        'SteamDatagramHostedAddress'
       );

      // Records that are not part of steam_api.json but still need verifying.
      ExtraVerifiableStructNames:array[0..2] of TText=
       (
        'CGameID',
        'CSteamID',
        'CallbackMsg_t'
      ...
      // Records whose size is verifiable but whose field offsets are not, because steam_api.json lists
      // union members under names that C cannot reach directly. SteamNetworkingConfigValue_t wraps its
      // union in a named member m_val, so the real C spelling is m_val.m_int64 rather than m_int64.
      OffsetUnverifiableStructNames:array[0..0] of TText=
       (
        'SteamNetworkingConfigValue_t'
       );
...
// Valve's own packing canary from steamclientpublic.h. Its size says which of the two callback packing
// regimes is in effect: 24 bytes under VALVE_CALLBACK_PACK_SMALL, 32 under VALVE_CALLBACK_PACK_LARGE.
// The SDK asserts this at compile time, and the layout verifier checks the Pascal side against it.
procedure EmitPackingSentinelType;
begin

 SetPackingMode(PackingModePlatform);

 Emit('type { TValvePackingSentinel_t }');
 Emit('     PPValvePackingSentinel_t=^PValvePackingSentinel_t;');
 Emit('     PValvePackingSentinel_t=^TValvePackingSentinel_t;');
 Emit('     TValvePackingSentinel_t=record');
 Emit('      m_u32:TSteamUInt32;');
 Emit('      m_u64:TSteamUInt64;');
 Emit('      m_u16:TSteamUInt16;');
 Emit('      m_d:TSteamDouble;');
 Emit('     end;');
 EmitEmptyLine;
...
// The verifier is a pair of programs that print sizeof for every record and offsetof for every field,
// one built from the real C++ headers and one from the generated unit. Comparing their output proves
// the layout instead of trusting it, which matters because the two packing regimes of the Steamworks
// headers hide different mistakes and because a Pascal compiler does not always agree with a C one
// about the alignment of an embedded packed record.

// The name the verifier has to use in Pascal, which is the view record for the two types whose declared
// type is byte array storage.
function GetVerifierPascalTypeName(const aName:TText):TText;
begin
 if IsInTextArray(aName,ViewedStructNames) then begin
  result:='T'+aName+'View';
 end else begin
  result:='T'+aName;
 end;
...
function IsVerifiableStruct(const aName:TText):boolean;
begin
 result:=not IsInTextArray(aName,IncompleteStructNames);
end;

function AreFieldOffsetsVerifiable(const aName:TText):boolean;
begin
 result:=IsVerifiableStruct(aName) and not IsInTextArray(aName,OffsetUnverifiableStructNames);
end;

procedure EmitLayoutDumperCPlusPlus;
var StructIndex,FieldIndex,Index:TPasJSONSizeInt;
begin

 Emit('// This file is generated by the steamapi2pas tool. Do not edit it, edit the generator instead.');
 Emit('//');
 Emit('// Prints the size of every Steamworks record and the offset of every field, as the C++ compiler');
 Emit('// sees them. The Pascal counterpart prints the same list, and the two outputs have to match');
 Emit('// exactly. Build it once per target, since Linux, macOS and FreeBSD pack these records with 4');
 Emit('// bytes while Windows packs them with 8.');
 EmitEmptyLine;
 Emit('#include <cstdio>');
 Emit('#include <cstddef>');
 EmitEmptyLine;
 // matchmakingtypes.h hides fields behind a plain private:, and offsetof needs to reach them. This is
 // layout neutral on every compiler in use, which the size comparison itself confirms.
 Emit('// matchmakingtypes.h uses a plain private:, and offsetof has to reach those fields.');
 Emit('#define private public');
 Emit('#define protected public');
 EmitEmptyLine;
 Emit('#include "steam_api.h"');
 Emit('#include "isteamgameserver.h"');
 Emit('#include "isteamgameserverstats.h"');
 Emit('#include "steamnetworkingfakeip.h"');
 Emit('#include "isteamdualsense.h"');
 EmitEmptyLine;
 Emit('int main()');
 Emit('{');

 // Valve's own compile time assertion, as a canary that the expected packing regime is in effect.
 Emit('  printf("ValvePackingSentinel_t %zu\n",sizeof(ValvePackingSentinel_t));');

 for Index:=Low(ExtraVerifiableStructNames) to High(ExtraVerifiableStructNames) do begin
  Emit('  printf("'+ExtraVerifiableStructNames[Index]+' %zu\n",sizeof('+ExtraVerifiableStructNames[Index]+'));');
 end;

 for StructIndex:=0 to length(StructItems)-1 do begin
  if IsVerifiableStruct(StructItems[StructIndex].Name) then begin
   Emit('  printf("'+StructItems[StructIndex].Name+' %zu\n",sizeof('+StructItems[StructIndex].Name+'));');
  end;
 end;

 Emit('  printf("CallbackMsg_t.m_hSteamUser %zu\n",offsetof(CallbackMsg_t,m_hSteamUser));');
 Emit('  printf("CallbackMsg_t.m_iCallback %zu\n",offsetof(CallbackMsg_t,m_iCallback));');
 Emit('  printf("CallbackMsg_t.m_pubParam %zu\n",offsetof(CallbackMsg_t,m_pubParam));');
 Emit('  printf("CallbackMsg_t.m_cubParam %zu\n",offsetof(CallbackMsg_t,m_cubParam));');

 for StructIndex:=0 to length(StructItems)-1 do begin
  if AreFieldOffsetsVerifiable(StructItems[StructIndex].Name) then begin
   for FieldIndex:=0 to length(StructItems[StructIndex].Fields)-1 do begin
    Emit('  printf("'+StructItems[StructIndex].Name+'.'+StructItems[StructIndex].Fields[FieldIndex].Name+' %zu\n",'+
         'offsetof('+StructItems[StructIndex].Name+','+StructItems[StructIndex].Fields[FieldIndex].Name+'));');
   end;
  end;
 end;
...
procedure EmitLayoutDumperPascal;
var StructIndex,FieldIndex,Index:TPasJSONSizeInt;
    TypeName:TText;
begin
 ...
 Emit('** This file is generated by the steamapi2pas tool. Do not edit it, edit the generator instead.');
 Emit('**');
 Emit('** The Pascal half of the layout verifier. Prints the same list as the C++ half, see there.');
 Emit('*)');
 Emit('program steamworkslayoutdumppas;');
 Emit('{$ifdef fpc}');
 Emit(' {$mode delphi}');
 Emit('{$endif}');
 Emit('{$apptype console}');
 EmitEmptyLine;
 Emit('uses '+UnitName+';');
 EmitEmptyLine;

 // A field of a procedural type does not yield its address through @, it yields its value, so every
 // offset goes through an untyped var parameter instead.
 Emit('// An untyped var parameter is always passed by reference, so @ yields the field address here.');
 Emit('// A procedural field would otherwise yield its value rather than its address.');
 Emit('function FieldAddress(var aField):TSteamPtrUInt;');
 Emit('begin');
 Emit(' result:=TSteamPtrUInt(@aField);');
 Emit('end;');
 EmitEmptyLine;

 Emit('var');
 for StructIndex:=0 to length(StructItems)-1 do begin
  if AreFieldOffsetsVerifiable(StructItems[StructIndex].Name) and (length(StructItems[StructIndex].Fields)>0) then begin
   Emit(' Record'+StructItems[StructIndex].Name+':'+GetVerifierPascalTypeName(StructItems[StructIndex].Name)+';');
  end;
 end;
 Emit(' RecordCallbackMsg_t:TCallbackMsg_t;');
 EmitEmptyLine;

 Emit('begin');
 EmitEmptyLine;

 Emit(' WriteLn(''ValvePackingSentinel_t '',SizeOf(TValvePackingSentinel_t));');

 for Index:=Low(ExtraVerifiableStructNames) to High(ExtraVerifiableStructNames) do begin
  Emit(' WriteLn('''+ExtraVerifiableStructNames[Index]+' '',SizeOf(T'+ExtraVerifiableStructNames[Index]+'));');
 end;

 for StructIndex:=0 to length(StructItems)-1 do begin
  if IsVerifiableStruct(StructItems[StructIndex].Name) then begin
   Emit(' WriteLn('''+StructItems[StructIndex].Name+' '',SizeOf('+GetVerifierPascalTypeName(StructItems[StructIndex].Name)+'));');
  end;
 end;

 Emit(' WriteLn(''CallbackMsg_t.m_hSteamUser '',FieldAddress(RecordCallbackMsg_t.m_hSteamUser)-TSteamPtrUInt(@RecordCallbackMsg_t));');
 Emit(' WriteLn(''CallbackMsg_t.m_iCallback '',FieldAddress(RecordCallbackMsg_t.m_iCallback)-TSteamPtrUInt(@RecordCallbackMsg_t));');
 Emit(' WriteLn(''CallbackMsg_t.m_pubParam '',FieldAddress(RecordCallbackMsg_t.m_pubParam)-TSteamPtrUInt(@RecordCallbackMsg_t));');
 Emit(' WriteLn(''CallbackMsg_t.m_cubParam '',FieldAddress(RecordCallbackMsg_t.m_cubParam)-TSteamPtrUInt(@RecordCallbackMsg_t));');

 for StructIndex:=0 to length(StructItems)-1 do begin
  if AreFieldOffsetsVerifiable(StructItems[StructIndex].Name) then begin
   TypeName:='Record'+StructItems[StructIndex].Name;
   for FieldIndex:=0 to length(StructItems[StructIndex].Fields)-1 do begin
    Emit(' WriteLn('''+StructItems[StructIndex].Name+'.'+StructItems[StructIndex].Fields[FieldIndex].Name+' '','+
         'FieldAddress('+TypeName+'.'+StructItems[StructIndex].Fields[FieldIndex].Name+')-TSteamPtrUInt(@'+TypeName+'));');
   end;
  end;
 end;
...
// Writes whatever the given emitter produced into its own file.
procedure WriteEmittedFile(const aFileName:string);
begin
 OutputStream.Seek(0,soBeginning);
 OutputStream.SaveToFile(aFileName);
 WriteLn('  "',aFileName,'", ',OutputStream.Size,' bytes');
end;

var InputFileName,OutputFileName,LayoutDumperCPlusPlusFileName,LayoutDumperPascalFileName:string;
...
begin
 ...
 // The two halves of the layout verifier are optional, since only the tools/steamworkslayoutverify
 // script needs them.
 if ParamCount>=4 then begin
  LayoutDumperCPlusPlusFileName:=ParamStr(3);
  LayoutDumperPascalFileName:=ParamStr(4);
 end else begin
  LayoutDumperCPlusPlusFileName:='';
  LayoutDumperPascalFileName:='';
 end;

 if not FileExists(InputFileName) then begin
  WriteLn('Error: "',InputFileName,'" not found');
  WriteLn('Usage: steamapi2pas [steam_api.json] [',UnitName,'.pas] [layoutdump.cpp layoutdump.dpr]');
  Halt(1);
 end;
 ...
 try
  ...
  try
   ...
   WriteEmittedFile(OutputFileName);
   WriteLn('  ',CountEmittedFunctionNames,' entry points');

  finally
   FreeAndNil(OutputStream);
  end;

  if Length(LayoutDumperCPlusPlusFileName)>0 then begin

   WriteLn('Generating the layout verifier . . .');

   OutputStream:=TMemoryStream.Create;
   try
    EmitLayoutDumperCPlusPlus;
    WriteEmittedFile(LayoutDumperCPlusPlusFileName);
   finally
    FreeAndNil(OutputStream);
   end;
   ...
   OutputStream:=TMemoryStream.Create;
   try
    EmitLayoutDumperPascal;
    WriteEmittedFile(LayoutDumperPascalFileName);
   finally
    FreeAndNil(OutputStream);
   end;
