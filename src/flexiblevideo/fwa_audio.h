 ...
 *                                                                            *
 ******************************************************************************/
// fwa_audio.h — the "FWAC" audio sub-codec interface for the FVD container. The implementation in fwa_audio.c is
// the Flexible Wavelet Audio (FWA) codec with its standalone main()/CLI guarded out (FWA_NO_MAIN); it is a
// separate translation unit, so all of its internals stay file-local static and nothing clashes with fvdref.c.
// fvdenc.c calls fwa_encode, fvdplay.c calls fwa_decode. The encoded stream is self-describing (channels / sample
