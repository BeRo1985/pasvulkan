 ...
 ******************************************************************************/
/*
 * fvdplay.c — windowed FVD player.
 *
 * Reads a .fvd container (per-frame index + the coded frames + optional audio), decodes each frame on the GPU
 ...
 *
 *     ./fvdplay file.fvd               present in a window
 *     ./fvdplay file.fvd --decode-to=out.avi   decode the whole stream to an AVI and exit
 *     ./fvdplay file.fvd --verify      GPU-vs-CPU decode PSNR self-check, then exit
 */
#define FVD_NO_MAIN
#include "fvdref.c"
...
// .fvd container header (binary-compatible with fvdenc's writer). The color block is CICP
...
typedef struct {
  ...
  void *mv_map;
  VkBuffer alpha_mv_buffer;   // own-alpha-MV (MCTF): the alpha plane's OWN per-pair temporal MVs (used for plane 3 when the frame is own)
  void *alpha_mv_map;
...
static void decode3d_spatial(const Decode3D *d, int buf, int slot) {
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {
    ...
    int block_workgroups = (g_block_size == 128) ? plane_blocks : ((plane_blocks + 63) / 64);   // COOP (128): bitplane_unpack runs one WORKGROUP per block (block = gl_WorkGroupID.x), so dispatch plane_blocks (the I/P path does this; the 3D-DWT spatial path was missing it -> most 128-blocks went undecoded)
    ...
    // coefficients -> garbage. Clear it first. (Luma always carries energy so it masked this; flat chroma exposed it.)
    ...
    if (!d->lossless) {
      if (d->aq_enabled) {   // GPU AQ: modulate this subband's base step (aq_base_buffer) by its tile map into step_buffer first
        int32_t aq_push[5] = { pw, ph, d->levels, d->aq_cols, d->aq_rows };
        vkCmdBindPipeline(d->cmd, VK_PIPELINE_BIND_POINT_COMPUTE, d->apply_aq);
        vkCmdBindDescriptorSets(d->cmd, VK_PIPELINE_BIND_POINT_COMPUTE, d->layout_apply_aq, 0, 1, &d->apply_aq_pf[plane], 0, 0);
        vkCmdPushConstants(d->cmd, d->layout_apply_aq, VK_SHADER_STAGE_COMPUTE_BIT, 0, 20, aq_push);
...
static void decode3d_mctf_inverse(const Decode3D *d, int buf, int gop_count, const int *frame_mv, const int *alpha_frame_mv) {
  int luma_blocks = d->motion_blocks_x * d->motion_blocks_y;
  int plane_count = d->has_alpha ? (g_num_planes + 1) : g_num_planes;   // alpha plane 3 rides the MC-Haar (shared luma MVs, or its OWN when alpha_frame_mv != NULL)
  ...
  for (int l = count - 1; l >= 0; l--) {
    ...
    for (int k = 0; k < low_count; k++) {
      ...
      if (((2 * k) + 1) < level_len) {
        ...
        if (alpha_frame_mv) {   // own-alpha-MV: this pair's OWN alpha temporal MVs (plane 3 binds alpha_mv_buffer below)
          memcpy(d->alpha_mv_map, &alpha_frame_mv[((size_t)(low_count + k) * luma_blocks) * 2], (size_t)luma_blocks * 2 * 4);
        }
        for (int plane = 0; plane < plane_count; plane++) {
          ...
          // mc: warp gop@low(k) by this pair's MVs (alpha uses its OWN field when this frame is own) -> mctf_pred[plane]
          bind_storage_buffers_offset(d->mctf_mc[plane],
            (VkBuffer[]){ d->gop_buffer[buf][plane], ((alpha_frame_mv && (plane == 3)) ? d->alpha_mv_buffer : d->mv_buffer), d->mctf_pred[plane] }, (VkDeviceSize[]){ low_off, 0, 0 }, 3);
...
static void decode3d_finish_gop(const Decode3D *d, int buf, int gop_count, int *const *mctf_mv, int *const *alpha_mctf_mv) {
  if (d->mctf) {
    decode3d_mctf_inverse(d, buf, gop_count, mctf_mv[buf], (d->has_alpha && alpha_mctf_mv) ? alpha_mctf_mv[buf] : NULL);
  } else {
...
static void upload_subband(FILE *file, const FrameEntry *index, uint32_t source_index, uint8_t **frame_buffer, size_t *frame_buffer_capacity,
  ...
  if (!lossless) {
    ...
    for (int plane = 0; plane < g_num_planes; plane++) {   // per-plane quant steps (chroma is subsampled), as the encoder builds them
      ...
      build_quantization_steps(step, pw, ph, levels, effective_quality);   // BASE step (no AQ); under --aq the GPU pass modulates it
  ...
  if (g_has_alpha && out_alpha_qp) {   // parse + upload the appended alpha section: its block data goes right after the
    ...
    int alpha_own = (index[source_index].alpha_mv_mode == 1);   // own-alpha-MV (MCTF): this high-pass frame carries its own alpha temporal MV blob
    const uint8_t *alpha_mv_blob = NULL;
    uint32_t alpha_mv_blob_length = 0;
    const uint8_t *alpha_data = parse_alpha_section(alpha_section, frame_len - (size_t)(alpha_section - *frame_buffer), a_off, a_block_count, &alpha_qp, &alpha_table, &alpha_table_length, &alpha_data_length, alpha_own, &alpha_mv_blob, &alpha_mv_blob_length);
    if (!alpha_data) {
      die("corrupt alpha section (3D-DWT subband)");
    }
    if (alpha_mctf_mv_out) {   // fill this frame's alpha temporal MV slab: own MVs from the blob, or (shared high-pass frame) a copy of the luma MVs so the alpha MC-Haar rides luma; low-pass frames have no mc
      int motion_blocks_x = ((width + g_motion_block) - 1) / g_motion_block, motion_blocks_y = ((height + g_motion_block) - 1) / g_motion_block;
      if (alpha_own && alpha_mv_blob_length) {
        if (g_mv_codec == 1) {
          mv_blob_decode_range(alpha_mv_blob, (size_t)alpha_mv_blob_length, 0, NULL, 0, alpha_mctf_mv_out, 0, NULL, 0, motion_blocks_x, motion_blocks_y);
        } else {
          BitReader a_mvr;
          bitreader_init(&a_mvr, alpha_mv_blob, (size_t)alpha_mv_blob_length);
          decode_motion_vectors(&a_mvr, alpha_mctf_mv_out, motion_blocks_x, motion_blocks_y);
        }
      } else if (mctf_mv_out && (mv_length > 0)) {
        memcpy(alpha_mctf_mv_out, mctf_mv_out, (size_t)(motion_blocks_x * motion_blocks_y) * 2 * 4);
      }
    }
...
static void cpu_decode_alpha_section(FILE *file, const FrameEntry *index, uint32_t entry_index,
  ...
  if (g_bframe_dct) {   // only DCT-B color writes a rANS entropy section before the alpha; wavelet-B + 3D-DWT have none
    ...
    if (g_quadtree) {   // DCT-B quadtree: the per-plane partition codes follow the entropy section -> skip them to reach the alpha section
      const uint8_t *qt_blobs[MAX_PLANES];
      int qt_regions = qt_region_count(width) * qt_region_count(height);
      alpha_section_ptr = parse_partition_section(alpha_section_ptr, qt_regions, 3, qt_blobs);
    }
  }
  int alpha_qp;
  const uint8_t *alpha_table;
...
int main(int argc, char **argv) {
  init_exe_dir();   // resolve shaders relative to the binary, so fvdplay works from any working directory
  if (argc < 2) {
    fprintf(stderr,
      "usage: %s file.fvd [flags...]\n"
  ...
  if (((fread(&header, sizeof header, 1, file) != 1) || memcmp(header.magic, "FVDC", 4)) || (header.version != 1)) {
    die("not a .FVD (or unsupported version)");
  }
  ...
  g_motion_mode = (header.mv_codec >> 1) & 15;   // bits1-4 = sub-pel mode (bits1-2 = interp filter 0 bilinear/1 6-tap/2 8-tap, bits3-4 = MV precision 0 half/1 quarter/2 eighth[reserved]/3 amvr[reserved]). Old files = 0 = bilinear + half-pel.
  ...
  g_quadtree = (header.color_flags & 8) != 0;   // bit3 = adaptive quadtree transform sizes (decoded on the CPU oracle for now)
  ...
  uint64_t fvd_file_size = (uint64_t)ftello(file);
  for (uint32_t c = 0; c < header.frame_count; c++) {
    ...
    if ((e->offset > fvd_file_size) || (e->size > (fvd_file_size - e->offset))) {
  ...
  printf("  color: %s / %s / %d-bit %s%s%s / %s\n", primaries_name, transfer_name, header.bit_depth,
         (header.color_flags & 1) ? "HDR" : "SDR", (header.color_flags & 2) ? " +HDR10 metadata" : "",
         (header.color_flags & 4) ? " +alpha" : "", chroma_name);
  int is_hdr = (header.color_flags & 1) != 0;
  g_has_alpha = (header.color_flags & 4) != 0;   // bit2 = has_alpha: decode the appended section into the rgba alpha lane
  ...
  SDL_Window *window = SDL_CreateWindow("fvdplay", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height,
  ...
  // Add VK_EXT_swapchain_colorspace if the loader has it, so the surface can report HDR/scRGB color
  ...
  if ((device_properties.limits.maxComputeWorkGroupInvocations < 256) || (device_properties.limits.maxComputeSharedMemorySize < 16384)) {
    fprintf(stderr, "GPU below the FVD decode floor: needs >= 256 compute invocations and >= 16 KiB shared memory "
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {
    ...
    create_buffer((size_t)RANS_GPU_TABLE_UINTS * 4, HOST_VISIBLE_COHERENT, &table_buffer[plane], &table_memory[plane]);
  }
  ...
  if (g_aq_enabled) {   // color: per-frame tile codes + a per-plane cached base step
    ...
    for (int plane = 0; plane < g_num_planes; plane++) {
      create_buffer(plane_bytes, HOST_VISIBLE_COHERENT, &aq_base_buffer[plane], &aq_base_memory[plane]);
      VK_CHECK(vkMapMemory(device, aq_base_memory[plane], 0, VK_WHOLE_SIZE, 0, &aq_base_map[plane]));
    }
  }
  ...
  // region count. (Used by the QT decode shaders — sub-step 4b; created green here so the upload path can fill it.)
  VkBuffer partition_buffer = 0;
  VkDeviceMemory partition_memory = 0;
  void *partition_map = 0;
  ...
  // Leaf list for dct_inv_qt: 3 uints (x,y,size) per leaf, shared across planes (4:4:4 same partition). Worst case
  // = every region fully split into 8s = 16 leaves/region. Rebuilt + uploaded per frame from the partition.
  // Per-PLANE leaf lists: the encoder decides the partition independently per plane (luma/Co/Cg differ), so each
  // plane's inverse-DCT dispatch needs its own leaf list (3 uints x,y,size per leaf).
  VkBuffer leaf_list_buffer[MAX_PLANES] = { 0 };
  VkDeviceMemory leaf_list_memory[MAX_PLANES] = { 0 };
  void *leaf_list_map[MAX_PLANES] = { 0 };
  int qt_leaf_count[MAX_PLANES] = { 0 };   // leaves in the current frame's partition, per plane (set per frame)
  ...
  // Deblock: per-plane 8x8-cell leaf-id map (one int per cell), rebuilt + uploaded per frame, so the GPU deblock
  // filters only true leaf boundaries (matches the CPU deblock_plane cell map). Shared layout as the quadtree.
  VkBuffer cell_leaf_buffer[MAX_PLANES] = { 0 };
  VkDeviceMemory cell_leaf_memory[MAX_PLANES] = { 0 };
  void *cell_leaf_map[MAX_PLANES] = { 0 };
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {
    ...
    VK_CHECK(vkMapMemory(device, table_memory[plane], 0, VK_WHOLE_SIZE, 0, &table_map[plane]));
  }
  ...
  if (g_has_alpha) {
    ...
    VK_CHECK(vkMapMemory(device, table_memory[3], 0, VK_WHOLE_SIZE, 0, &table_map[3]));
  }
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {   // per-plane quant map (chroma subsampled -> its own subband layout); 4:4:4 -> all identical
    ...
    build_spatial_quant_steps(step, plane_w, plane_h, levels, quality);   // BASE step (no AQ); DCT path builds the JPEG quant matrix
    if (g_spatial_dct && (plane > 0)) {
      build_dct_quant_matrix_chroma(step, plane_w, plane_h, quality);   // dedicated chroma quant matrix (matches the encoder)
    }
    memcpy(step_map[plane], step, (size_t)(plane_w * plane_h) * 4);
    if (g_aq_enabled) {   // seed the AQ base; step_buffer's own content is overwritten by apply_tile_aq.comp each frame
      memcpy(aq_base_map[plane], step, (size_t)(plane_w * plane_h) * 4);
    }
  }
  ...
  // compatible); the color compute stores raw gamma bytes through a UNORM view of the same image. HDR stays linear
  ...
  view_info.format = decode_storage_format;   // UNORM view (SDR) so the color compute stores raw gamma bytes; FP16 for HDR
  ...
  // ---- pipelines (same decode chain as fvddec) ----
  ...
  VkDescriptorSetLayout layout_rans4 = create_descriptor_set_layout(4, 0);   // DCT rANS unpack: data + offset + coeff + table
  VkPipelineLayout pipeline_layout_rans_unpack = create_pipeline_layout(layout_rans4, 16);   // push { width, height, block_size, data_length }
  ...
  VkPipelineLayout pipeline_layout_rans_unpack_qt = g_quadtree ? create_pipeline_layout(layout_rans5_qt, 24) : 0;   // push { width, height, block_size, data_length, qt_offset, regions_x }
  VkPipeline pipeline_rans_unpack_qt = g_quadtree ? create_compute_pipeline("shaders/rans_unpack_qt.spv", pipeline_layout_rans_unpack_qt) : 0;
  VkDescriptorSetLayout layout_dct_qt = g_quadtree ? create_descriptor_set_layout(2, 0) : 0;   // { coeff, leaf_list }
  VkPipelineLayout pipeline_layout_dct_qt = g_quadtree ? create_pipeline_layout(layout_dct_qt, 8) : 0;   // push { width, leaf_count }
  VkPipeline pipeline_dct_inv_qt = g_quadtree ? create_compute_pipeline("shaders/dct_inv_qt.spv", pipeline_layout_dct_qt) : 0;
  // In-loop deblock (created only when g_deblock): { plane, cell_leaf }, 28-byte push {w,h,cells_x,alpha,beta,tc,direction}.
  ...
  // color_alpha variant: bindings 0,1,2 = color coeff buffers, 3 = output image, 4 = alpha coeff buffer.
  VkDescriptorSetLayout layout_color_alpha = 0;
  VkPipelineLayout pipeline_layout_color_alpha = 0;
  VkPipeline pipeline_color_alpha = 0;
  if (g_has_alpha) {
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(device, &ca_info, 0, &layout_color_alpha));
    pipeline_layout_color_alpha = create_pipeline_layout(layout_color_alpha, 24);   // same push constants as color.comp
    pipeline_color_alpha = create_compute_pipeline("shaders/color_alpha.spv", pipeline_layout_color_alpha);
  }
  VkPipelineLayout pipeline_layout_color_hdr = create_pipeline_layout(layout_color, 32);   // {width, height, exposure, transfer, shift_x, shift_y, small_w, small_h}
  VkPipeline pipeline_color_hdr = create_compute_pipeline("shaders/color_hdr.spv", pipeline_layout_color_hdr);
  VkPipeline pipeline_color_hdr_scrgb = create_compute_pipeline("shaders/color_hdr_scrgb.spv", pipeline_layout_color_hdr);   // FP16 scRGB output
  // HDR/scRGB alpha variants: the SAME 5-binding layout_color_alpha (set_color_alpha works for both SDR and HDR alpha),
  // but with the HDR 32-byte push. Only the final color write differs (the alpha plane goes into the swapchain A).
  VkPipelineLayout pipeline_layout_color_hdr_alpha = 0;
  VkPipeline pipeline_color_hdr_alpha = 0;
  VkPipeline pipeline_color_hdr_scrgb_alpha = 0;
  if (g_has_alpha) {
    pipeline_layout_color_hdr_alpha = create_pipeline_layout(layout_color_alpha, 32);
    pipeline_color_hdr_alpha = create_compute_pipeline("shaders/color_hdr_alpha.spv", pipeline_layout_color_hdr_alpha);
    pipeline_color_hdr_scrgb_alpha = create_compute_pipeline("shaders/color_hdr_scrgb_alpha.spv", pipeline_layout_color_hdr_alpha);
  }
  // window-present alpha composite: blend the decoded alpha over a checkerboard / solid color in-place in decode_image
  ...
  if (g_has_alpha) {
    ...
    pipeline_composite = create_compute_pipeline("shaders/fvd_composite_present.spv", pipeline_layout_composite);
  }
  ...
  VkDescriptorSet set_dct_qt[MAX_PLANES] = { 0 };           // quadtree inverse DCT: { coeff, leaf_list }
  VkDescriptorSet set_deblock[MAX_PLANES] = { 0 };          // deblock: { coeff (the reconstruction), cell_leaf }
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {
    ...
    set_rans_unpack[plane] = allocate_descriptor_set(descriptor_pool, layout_rans4);
    bind_storage_buffers(set_rans_unpack[plane], (VkBuffer[]){ data_buffer, offset_buffer[plane], coeff_buffer[plane], table_buffer[plane] }, 4);
    ...
    if (g_aq_enabled) {
      set_apply_aq[plane] = allocate_descriptor_set(descriptor_pool, layout_4_buffers);
      // Default (I/P + colordiff) binding: modulate the host-rebuilt base into step_buffer. The B-decode path rebinds
      // this per frame to read the step_cache instead (an I/P stream never runs that, so this static bind persists).
      bind_storage_buffers(set_apply_aq[plane], (VkBuffer[]){ aq_base_buffer[plane], tile_codes_buffer, weight_lut_buffer, step_buffer[plane] }, 4);
    }
    ...
    if (g_quadtree) {
      set_rans_unpack_qt[plane] = allocate_descriptor_set(descriptor_pool, layout_rans5_qt);
      bind_storage_buffers(set_rans_unpack_qt[plane], (VkBuffer[]){ data_buffer, offset_buffer[plane], coeff_buffer[plane], table_buffer[plane], partition_buffer }, 5);
      set_dct_qt[plane] = allocate_descriptor_set(descriptor_pool, layout_dct_qt);
      bind_storage_buffers(set_dct_qt[plane], (VkBuffer[]){ coeff_buffer[plane], leaf_list_buffer[plane] }, 2);
    }
    if (g_deblock) {
      set_deblock[plane] = allocate_descriptor_set(descriptor_pool, layout_deblock);
      bind_storage_buffers(set_deblock[plane], (VkBuffer[]){ coeff_buffer[plane], cell_leaf_buffer[plane] }, 2);
    }
  }
  ...
  VkDescriptorSet set_color_alpha = 0;   // bound after set_color (it needs the decode image view)
  ...
  set_color = allocate_descriptor_set(descriptor_pool, layout_color);
  {
    ...
    VkDescriptorImageInfo color_image = { 0, decode_view, VK_IMAGE_LAYOUT_GENERAL };
    VkWriteDescriptorSet writes[4] = { 0 };
    for (int i = 0; i < 3; i++) {
  ...
  if (g_has_alpha) {   // set_color_alpha: 3 color planes (0,1,2) + output image (3) + the alpha plane (4)
    set_color_alpha = allocate_descriptor_set(descriptor_pool, layout_color_alpha);
    ...
    int ca_bind[4] = { 0, 1, 2, 4 };   // color buffers at 0,1,2; alpha buffer at 4 (binding 3 is the image)
    for (int i = 0; i < 4; i++) {
      ca_w[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
      ca_w[i].dstSet = set_color_alpha;
  ...
  // color-converted and presented in order. To hide the per-GOP decode cost, two GOP buffers ping-pong:
  ...
  int *alpha_mctf_mv[2] = { NULL, NULL };   // own-alpha-MV (MCTF): per-ping-pong-buffer alpha temporal MV field (own MVs, or luma copies for shared high-pass frames)
  ...
  if (mode_3ddwt && g_mctf) {
    ...
    for (int buffer = 0; buffer < 2; buffer++) {
      ...
      if (g_has_alpha) {   // own-alpha-MV: a parallel per-buffer alpha MV field
        alpha_mctf_mv[buffer] = checked_malloc(((size_t)gop_capacity * motion_blocks_x * motion_blocks_y * 2) * sizeof(int));
      }
    }
  }
  if (mode_3ddwt) {
  ...
  if (verify || g_quadtree) {   // quadtree: the GPU path can't decode it yet, so the CPU oracle drives the display too
  ...
  if (mode_3ddwt) {
    ...
    d3d.mv_map = mv_map;
    d3d.alpha_mv_buffer = alpha_mv_buffer;   // own-alpha-MV (MCTF): the alpha plane's own per-pair temporal MVs (filled in decode3d_mctf_inverse)
    d3d.alpha_mv_map = alpha_mv_map;
    for (int plane = 0; plane < g_num_planes; plane++) {
      d3d.unpack_pf[plane] = set_unpack_pf[plane];
      d3d.dequant_pf[plane] = set_dequant_pf[plane];
      d3d.apply_aq_pf[plane] = g_aq_enabled ? set_apply_aq[plane] : 0;   // GPU AQ pass set (I/P's, unused in 3D-DWT mode)
  ...
  int composite_bg_mode = (getenv("FVD_BLENDBG") != NULL) ? 1 : 0;   // window alpha composite background: 0 = checkerboard, 1 = solid color ('G' toggles)
  int fvd_time = (getenv("FVD_TIME") != NULL);   // FVD_TIME=1 -> print B-decode CPU-prep vs GPU-busy breakdown at exit
  ...
  for (uint32_t frame_index = 0; (frame_index < header.frame_count) && !quit; frame_index++) {
    double loop_t0 = fvd_time ? now_milliseconds() : 0.0;   // present-loop per-frame timer (both paths)
    SDL_Event event;
    ...
    double loop_tf = fvd_time ? now_milliseconds() : 0.0;
    ...
    // behind), then begin prefetching the GOP after it. The per-frame present below color-converts the
    ...
    if (mode_3ddwt && (frame_index >= cur_gop_start + (uint32_t)cur_gop_count)) {
      while (!pf_done) {   // the prefetch didn't keep up (rare): finish it now
        ...
        if (pf_step >= pf_gop_count) {
          decode3d_wait(&d3d);
          decode3d_finish_gop(&d3d, pf_buf, pf_gop_count, mctf_mv, alpha_mctf_mv);
          pf_done = 1;
        }
      }
    ...
    if (!mode_3ddwt && !has_bframes) {
      ...
      if (g_spatial_dct) {
        ...
        const uint8_t *after_entropy = parse_entropy_section(frame_data + data_length, g_num_planes, table_blobs, table_lens);
        alpha_section_start = after_entropy;   // DCT mode: alpha section follows the entropy tables
        for (int plane = 0; plane < g_num_planes; plane++) {
          rans_build_gpu_table(table_blobs[plane], table_lens[plane], (uint32_t *)table_map[plane]);
        }
        // Quadtree: the partition section follows the entropy section; upload it for the GPU quadtree decode.
        if (g_quadtree && partition_map) {
          const uint8_t *qt_blobs[MAX_PLANES];
          parse_partition_section(after_entropy, qt_regions_per_plane, g_num_planes, qt_blobs);
          for (int plane = 0; plane < g_num_planes; plane++) {
      ...
      if (g_has_alpha) {   // parse + upload the appended alpha section: its block data goes right after the color data in data_buffer
        ...
        alpha_is_rans = (alpha_table_length > 0);
    ...
    double loop_tp = fvd_time ? now_milliseconds() : 0.0;
    if (audio_device) {
    ...
    if (fvd_time) {
      loop_pace_ms += (now_milliseconds() - loop_tp);
    }
    ...
    if (!verify && !decode_to_path && !eval_mode) {   // decode-to/eval are headless too: no swapchain acquire / present
      ...
      double loop_taq = fvd_time ? now_milliseconds() : 0.0;
      ...
      if (fvd_time) {
        loop_acquire_ms += (now_milliseconds() - loop_taq);
      }
    ...
    if (use_gpu_bdecode) {
      ...
      while (gcursor < (int)header.frame_count) {
        ...
        double bdec_t0 = fvd_time ? now_milliseconds() : 0.0;   // CPU-prep timer (MV parse + rebinds + command recording)
        int c = gcursor;
        ...
        if ((g_bframe_dct && (entry->ref0 >= 0)) && (mv_length == 0)) {
          // CPU-oracle DCT-B (encode_frame_bidi) is zero-motion: it carries no MV blob and predicts straight
          // from the references (mc with mv=0 = identity). Clear the MV buffers the OBMC mc.comp reads so a
          // stale field from an earlier frame cannot leak in (mv1 in particular is never otherwise zeroed).
          // The GPU motion DCT-B path (mv_length > 0) decodes real MVs above and skips this.
          int mv_block_count = motion_blocks_x * motion_blocks_y;
          memset(mv_map, 0, (size_t)(mv_block_count * 2) * 4);
          if (entry->ref1 >= 0) {
            memset(mv1_map, 0, (size_t)(mv_block_count * 2) * 4);
          }
        }
        ...
        int is_phase2_b = (has_per_block_mode && (ref1_slot >= 0)) && (!g_bframe_dct || (mv_length > 0));   // per-block L0/L1/BI mode; zero-motion CPU-oracle DCT-B (mv_length==0) uses the global-weight blend, GPU motion DCT-B (mv_length>0) uses per-block mode like wavelet
        double bdec_ta = fvd_time ? now_milliseconds() : 0.0;   // end of parse (frame read + header + MV parse + uploads)
        for (int plane = 0; plane < g_num_planes; plane++) {
          if (is_pred) {
          ...
          if (g_bframe_dct && g_deblock) {   // DCT-B in-loop deblock: filter this frame's reconstructed DPB slot in place
        ...
        if (!lossless && g_bframe_dct) {
          // DCT-B: build this frame's DCT (or quadtree) quant step per plane into step_buffer (the partition + the
          // QP-cascaded quality change per frame, so no cross-frame cache), bind the dequant to it. Mirrors encode_frame_bidi.
          ...
          // Per-quality BASE step cache (content-independent, built once per distinct quality). Under --aq this is the
          // base apply_tile_aq.comp modulates on the GPU per frame; otherwise the dequant reads it directly. Either way
          // there is no per-frame O(width*height) CPU rebuild (the 4K bottleneck), and under --aq no CPU AQ pass at all.
          ...
          if ((idx < 0) && (step_cache_n < STEP_CACHE_MAX)) {
            ...
            for (int plane = 0; plane < g_num_planes; plane++) {
              ...
              build_quantization_steps(step, pw, ph, levels, q);   // base step; AQ (if any) is applied on the GPU below
          ...
          if (g_aq_enabled) {
            ...
            gdec_step_idx = -1;
          } else if ((idx >= 0) && (idx != gdec_step_idx)) {
            for (int plane = 0; plane < g_num_planes; plane++) {
        ...
        double bdec_tb = fvd_time ? now_milliseconds() : 0.0;   // end of descriptor rebinds; command recording follows
        ...
        for (int plane = 0; plane < g_num_planes; plane++) {
          ...
          if (g_bframe_dct && b_use_qt) {   // DCT-B quadtree: rANS unpack with the partition (reuses the I/P QT set/buffers)
            int32_t rans_push[6] = { plane_w, plane_h, g_block_size, (int)data_length, plane * qt_regions_per_plane, qt_region_count(plane_w) };
            vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_rans_unpack_qt);
            vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_rans_unpack_qt, 0, 1, &set_rans_unpack_qt[plane], 0, 0);
            vkCmdPushConstants(command_buffer, pipeline_layout_rans_unpack_qt, VK_SHADER_STAGE_COMPUTE_BIT, 0, 24, rans_push);
            vkCmdDispatch(command_buffer, plane_block_count, 1, 1);
          } else if (g_bframe_dct) {   // DCT-B uniform: rANS unpack
            int32_t rans_push[4] = { plane_w, plane_h, g_block_size, (int)data_length };
            vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_rans_unpack);
            vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_rans_unpack, 0, 1, &set_rans_unpack[plane], 0, 0);
            vkCmdPushConstants(command_buffer, pipeline_layout_rans_unpack, VK_SHADER_STAGE_COMPUTE_BIT, 0, 16, rans_push);
            vkCmdDispatch(command_buffer, plane_block_count, 1, 1);
          } else {
          ...
          if (!lossless) {
            ...
            if (g_aq_enabled) {   // GPU AQ: modulate the cached base step by this frame's tile map into step_buffer first
              int32_t aq_push[5] = { plane_w, plane_h, levels, aq_cols, aq_rows };
              vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_apply_aq);
              vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_apply_aq, 0, 1, &set_apply_aq[plane], 0, 0);
              vkCmdPushConstants(command_buffer, pipeline_layout_apply_aq, VK_SHADER_STAGE_COMPUTE_BIT, 0, 20, aq_push);
          ...
          if (g_bframe_dct && b_use_qt) {
            // DCT-B quadtree: one workgroup per leaf (this plane's host-built leaf list), size-parameterized inverse DCT.
            int32_t dct_push[2] = { plane_w, qt_leaf_count[plane] };
            ...
            vkCmdPushConstants(command_buffer, pipeline_layout_dct_qt, VK_SHADER_STAGE_COMPUTE_BIT, 0, 8, dct_push);
        ...
        if ((g_bframe_dct && g_deblock) && !lossless) {
          int dq = (int)entry->quality * (g_sample_white / 256);
          if (dq >= 1) {
            int alpha = dq;
            int beta = (dq >> 1) + 1;
            int tc = (dq >> 2) + 1;
            for (int plane = 0; plane < g_num_planes; plane++) {
              int plane_w = plane_width(plane, width), plane_h = plane_height(plane, height);
              int cells_x = plane_w / 8, cells_y = plane_h / 8;
              int32_t deblock_v[7] = { plane_w, plane_h, cells_x, alpha, beta, tc, 0 };
              ...
              vkCmdPushConstants(command_buffer, pipeline_layout_deblock, VK_SHADER_STAGE_COMPUTE_BIT, 0, 28, deblock_v);
              vkCmdDispatch(command_buffer, (uint32_t)(((cells_x - 1) + 7) / 8), (uint32_t)((plane_h + 7) / 8), 1);
              memory_barrier();   // vertical pass complete before the horizontal pass reads it
              ...
              memory_barrier();   // deblocked slot visible to CDEF + the display copy
            }
          }
        }
        ...
        double bdec_t1 = fvd_time ? now_milliseconds() : 0.0;   // CPU-prep done; the GPU-busy window starts here
    ...
    // Output image into GENERAL so the color shader can store into it.
    ...
    if (use_gpu_bdecode) {
      ...
      if (index[frame_index].quality != current_quality) {
        ...
        for (int plane = 0; plane < g_num_planes; plane++) {   // per-plane quant map (4:4:4 -> all identical)
          int plane_w = plane_width(plane, width), plane_h = plane_height(plane, height);
          build_spatial_quant_steps(step, plane_w, plane_h, levels, current_quality);   // BASE step (no AQ); DCT path = JPEG quant matrix
          if (g_spatial_dct && (plane > 0)) {
            build_dct_quant_matrix_chroma(step, plane_w, plane_h, current_quality);   // dedicated chroma quant matrix
          }
          // Under --aq this base feeds apply_tile_aq.comp (-> step_buffer); otherwise the dequant reads it directly.
          memcpy(g_aq_enabled ? aq_base_map[plane] : step_map[plane], step, (size_t)(plane_w * plane_h) * 4);
        }
      }
      for (int plane = 0; plane < g_num_planes; plane++) {
        ...
        // Entropy decode -> integer coefficients. DCT lossy: GPU rANS (one workgroup per coding tile); else bit-plane.
        if ((g_spatial_dct && !lossless) && g_quadtree) {
          // Adaptive quadtree: the QT rANS shader walks the partition (5th binding) + per-leaf zigzag.
          int32_t rans_push[6] = { plane_w, plane_h, g_block_size, (int)ip_block_data_length,
                                   plane * qt_regions_per_plane, qt_region_count(plane_w) };
          vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_rans_unpack_qt);
          vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_rans_unpack_qt, 0, 1, &set_rans_unpack_qt[plane], 0, 0);
          vkCmdPushConstants(command_buffer, pipeline_layout_rans_unpack_qt, VK_SHADER_STAGE_COMPUTE_BIT, 0, 24, rans_push);
          vkCmdDispatch(command_buffer, plane_block_count, 1, 1);
        } else if (g_spatial_dct) {   // DCT mode (lossy float-DCT + lossless integer-DCT) -> GPU rANS; wavelet mode -> bit-plane below
          int32_t rans_push[4] = { plane_w, plane_h, g_block_size, (int)ip_block_data_length };
          vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_rans_unpack);
          vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_rans_unpack, 0, 1, &set_rans_unpack[plane], 0, 0);
          vkCmdPushConstants(command_buffer, pipeline_layout_rans_unpack, VK_SHADER_STAGE_COMPUTE_BIT, 0, 16, rans_push);
          vkCmdDispatch(command_buffer, plane_block_count, 1, 1);
        ...
        if (g_quadtree && !lossless) {
          // Adaptive quadtree: rebuild this plane's per-leaf quant step from the partition (which changes every
          // frame), straight into step_map, bypassing AQ — exactly like the encoder (build_quant_quadtree there
          // overwrites the AQ step). chroma coarsening is applied by the dequant shader via chroma_multiplier.
          const int *quant_table = (plane == 0) ? JPEG_LUMA_QUANT : JPEG_CHROMA_QUANT;
          build_quant_quadtree(step, plane_w, plane_h, (const uint8_t *)partition_map + ((size_t)plane * qt_regions_per_plane), current_quality, quant_table);
          memcpy(step_map[plane], step, (size_t)(plane_w * plane_h) * 4);
        }
        if (!lossless) {
          ...
          float chroma_multiplier = (plane == 0) ? 1.0f : g_chroma_quant;
          memcpy(&dequant_push[1], &chroma_multiplier, sizeof(float));
          if (g_aq_enabled && !g_quadtree) {   // GPU AQ: modulate the cached base step by this frame's tile map into step_buffer first (quadtree builds its own step above)
            int32_t aq_push[5] = { plane_w, plane_h, levels, aq_cols, aq_rows };
            vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_apply_aq);
            vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_apply_aq, 0, 1, &set_apply_aq[plane], 0, 0);
            vkCmdPushConstants(command_buffer, pipeline_layout_apply_aq, VK_SHADER_STAGE_COMPUTE_BIT, 0, 20, aq_push);
        ...
        if ((g_spatial_dct && !lossless) && g_quadtree) {
          // Adaptive quadtree: one workgroup per leaf (from this plane's host-built leaf list), size-parameterized inverse DCT.
          int32_t dct_push[2] = { plane_w, qt_leaf_count[plane] };
          vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_dct_inv_qt);
          vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_dct_qt, 0, 1, &set_dct_qt[plane], 0, 0);
          vkCmdPushConstants(command_buffer, pipeline_layout_dct_qt, VK_SHADER_STAGE_COMPUTE_BIT, 0, 8, dct_push);
          vkCmdDispatch(command_buffer, qt_leaf_count[plane], 1, 1);
          memory_barrier();
        } else if (g_spatial_dct && !lossless) {
          // DCT: a single inverse 8x8 block DCT (no wavelet level loop / transpose).
          int32_t dct_push[2] = { plane_w, plane_h };
      ...
      // In-loop deblock (--deblock): filter the reconstructed YCoCg int plane (coeff_buffer) at leaf boundaries
      // (the per-frame cell_leaf map) before CDEF, then mirror the deblocked result into previous_buffer (the
      // reference) so display + reference stay in sync. Exact mirror of the CPU deblock_plane. V edges then H.
      if (g_deblock && !lossless) {
        int dq = current_quality * (g_sample_white / 256);
        if (dq >= 1) {
          int alpha = dq;
          int beta = (dq >> 1) + 1;
          int tc = (dq >> 2) + 1;
          for (int plane = 0; plane < g_num_planes; plane++) {
            ...
            vkCmdCopyBuffer(command_buffer, coeff_buffer[plane], previous_buffer[plane], 1, &db_copy);
            VkMemoryBarrier copy_to_shader = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, 0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT };
            vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &copy_to_shader, 0, 0, 0, 0);
          }
        }
      ...
      if (g_has_alpha) {   // GPU-decode the alpha plane (intra): unpack -> dequant (lossy) -> inverse DWT -> round, into coeff[3]
        ...
        if (alpha_is_rans) {   // DCT mode: GPU rANS entropy (one workgroup per coding tile, mirror of the color planes)
          ...
          vkCmdPushConstants(command_buffer, pipeline_layout_rans_unpack, VK_SHADER_STAGE_COMPUTE_BIT, 0, 16, a_rans_push);
          vkCmdDispatch(command_buffer, a_block_count, 1, 1);
        ...
        if (alpha_is_rans) {   // DCT mode: float dct_inv (lossy) / reversible integer dct_inv_int (lossless), into coeff[3]
          int32_t a_dct_push[2] = { aw, ah };
          vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, lossless ? pipeline_dct_inv_int : pipeline_dct_inv);
          vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_layout_dct, 0, 1, &set_row[3], 0, 0);
          vkCmdPushConstants(command_buffer, pipeline_layout_dct, VK_SHADER_STAGE_COMPUTE_BIT, 0, 8, a_dct_push);
          vkCmdDispatch(command_buffer, (aw + 7) / 8, (ah + 7) / 8, 1);
          memory_barrier();
        } else {   // wavelet mode: inverse-row + transpose level loop
    ...
    // Chroma upsample params for the color shader: shift + the stored Co/Cg (plane 1) dims. 4:4:4 ->
    ...
    if (is_hdr) {   // HDR present: scRGB FP16 (real HDR display) or PQ/HLG -> tonemap -> sRGB8 (SDR fallback)
      ...
      if (g_has_alpha) {   // write the decoded alpha plane into the HDR / scRGB swapchain A (set_color_alpha binds the alpha buffer at 4)
        present_pipeline = use_scrgb_output ? pipeline_color_hdr_scrgb_alpha : pipeline_color_hdr_alpha;
        present_layout = pipeline_layout_color_hdr_alpha;
      } else {
        present_pipeline = use_scrgb_output ? pipeline_color_hdr_scrgb : pipeline_color_hdr;
        present_layout = pipeline_layout_color_hdr;
      }
      vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, present_pipeline);
      vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, present_layout, 0, 1, g_has_alpha ? &set_color_alpha : &set_color, 0, 0);
      vkCmdPushConstants(command_buffer, present_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, 32, &hdr_push);
    } else {
      int32_t color_push[6] = { width, height, chroma_shift_x_value, chroma_shift_y_value, chroma_small_w, chroma_small_h };
      vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, g_has_alpha ? pipeline_color_alpha : pipeline_color);
      vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, g_has_alpha ? pipeline_layout_color_alpha : pipeline_layout_color, 0, 1, g_has_alpha ? &set_color_alpha : &set_color, 0, 0);
      vkCmdPushConstants(command_buffer, g_has_alpha ? pipeline_layout_color_alpha : pipeline_layout_color, VK_SHADER_STAGE_COMPUTE_BIT, 0, 24, color_push);
    }
    ...
    if (g_has_alpha && !use_scrgb_output && !(verify || decode_to_path || eval_mode)) {
      ...
      struct { int32_t width, height, mode, premultiplied; float checker_size, color_r, color_g, color_b; } comp_push = {
        width, height, composite_bg_mode, (header.color_flags & 8) ? 1 : 0, 16.0f, 0.10f, 0.12f, 0.18f };
    ...
    if (pipeline_dering && dering && !lossless && !verify) {
      // decode_image GENERAL (color/composite output) -> read by the dering shader; dering_image -> GENERAL (written)
    ...
    if (verify || decode_to_path || eval_mode) {
      ...
      if ((((g_quadtree && cpu_decode) && (readback_map != NULL)) && !has_bframes) && (header.prediction_method == 1)) {   // --cpu-decode forces the CPU oracle; default = the (now-correct) GPU quadtree path drives the readback
        // The GPU rANS/DCT path can't decode adaptive-quadtree frames yet; drive the readback (--decode-to /
      ...
      if (dump_first_frame && (frame_index == 0)) {   // dump GPU-decoded frame 0 (--dump): raw RGBA16F when scRGB, else 8-bit PPM
        if (use_scrgb_output) {
          FILE *f16 = fopen("/tmp/fvd_frame0.f16", "wb");
          if (f16) {
            fwrite(gpu_rgba, 8, (size_t)pixel_count, f16);
            fclose(f16);
          }
        } else {
          FILE *ppm = fopen("/tmp/fvd_frame0.ppm", "wb");
          if (ppm) {
            fprintf(ppm, "P6\n%d %d\n255\n", width, height);
          ...
          if (g_has_alpha) {   // also dump the GPU-decoded alpha lane (frame 0) for an external round-trip compare
            FILE *af = fopen("/tmp/fvd_gpu_alpha0.gray", "wb");
            if (af) {
              for (int p = 0; p < pixel_count; p++) {
      ...
      if (verify && !has_bframes) {   // B-stream: decode_image already IS the CPU decode_frame_bidi output -> validate via --decode-to + external PSNR
        ...
        if (dump_alpha && (frame_index == 0)) {   // --dump-alpha: the CPU-decoded alpha lane of frame 0 (raw bytes, for an external round-trip compare)
          FILE *alpha_file = fopen("/tmp/fvd_alpha0.gray", "wb");
          if (alpha_file) {
            for (int p = 0; p < pixel_count; p++) {
    ...
    double loop_td = fvd_time ? now_milliseconds() : 0.0;
    ...
    if (fvd_time) {
      loop_drain_ms += (now_milliseconds() - loop_td);
    }  // now ~0 — the GPU wait moved to the loop-top fence
