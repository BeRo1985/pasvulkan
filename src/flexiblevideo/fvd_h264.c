 ...
 ******************************************************************************/
/*
 * fvd_h264.c — H.264 HW-decode helper: GPU NV12->RGB + aspect-correct present.
 *
...
#include "fvd_h264.h"   // shared-handle context (audio is owned by fvdplay)
...
int run_h264_player(const FvdH264Context *ctx) {
  ...
  // verify is taken from ctx->verify; --verify on fvdplay drives it. ctx->decode_to exports to OpenDML AVI.
  if (ctx->decode_to) {
  ...
  // fvdplay (OGGV/QOAL/RPCM) and arrives via ctx; the loop below only paces to it and unpauses it.
  ...
  if (verify && verify_count) {
    ...
    FILE *blob_file = fopen("/tmp/fvd_verify.h264", "wb");
    if (blob_file) {
      fwrite(stream, 1, stream_length, blob_file);
      fclose(blob_file);
    }
    char command[4096];
    snprintf(command, sizeof command, "ffmpeg -v error -i /tmp/fvd_verify.h264 -frames:v %d -f rawvideo -pix_fmt rgb24 -", verify_count);
  ...
  // The video ran out first but the audio may still be playing (audio slightly longer, or trailing padding): hold the
  // last displayed frame on screen and keep the window responsive until the queued audio drains, so playback ends
  // smoothly instead of cutting both off the instant the video finishes. ESC / window-close still exits.
  ...
  // fvdplay owns the SDL window + audio device + SDL lifetime (shared) — don't tear them down here.
  return 0;
}
