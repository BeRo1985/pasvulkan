 ...
 ******************************************************************************/
/*
 * fvddec.c — GPU decode + validation harness.
 *
 * Includes fvdref.c as the codec / container format / CPU reference. For every frame it:
 ...
 *
 *     ./fvddec in.(mp4|y4m) [quality=8] [levels=5] [max_frames=4]
 */
#define FVD_NO_MAIN
#include "fvdref.c"
...
int main(int argc, char **argv) {
  ...
  // (mirrors fvdenc's parsing). When set, the codec runs the colordiff path (which subsamples Co/Cg).
  ...
  int use_colordiff = (g_chroma_format != 0) || g_spatial_dct;   // colordiff subsamples Co/Cg; the DCT fork always uses colordiff (rANS + entropy section), incl. 4:4:4. coefdiff stays the wavelet 4:4:4 path. (lossless resets this below)
  ...
  if (lossless) {
    ...
    if (!g_spatial_dct) {
      use_colordiff = 0;   // wavelet-mode lossless = coefdiff + bit-plane (FWV 4:4:4); DCT-mode lossless stays colordiff + rANS + integer-DCT
    }
  }
  ...
  printf("fvddec GPU: %s | %dx%d quality=%d levels=%d | %d blocks | chroma %s\n",
  ...
  void *data_map, *offset_map[MAX_PLANES], *step_map[MAX_PLANES], *table_map[MAX_PLANES], *readback_map;
  ...
  VkDescriptorSetLayout layout_4_buffers = create_descriptor_set_layout(4, 0);   // DCT rANS unpack: data + offset + coeff + table
  ...
  VkPipelineLayout pipeline_layout_rans_unpack = create_pipeline_layout(layout_4_buffers, 16);   // push { width, height, block_size, data_length }
  ...
  VkDescriptorSet set_unpack[MAX_PLANES], set_rans_unpack[MAX_PLANES], set_dequant[MAX_PLANES], set_coeff_to_scratch[MAX_PLANES], set_scratch_to_coeff[MAX_PLANES], set_row[MAX_PLANES];
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {
    ...
    set_rans_unpack[plane] = allocate_descriptor_set(descriptor_pool, layout_4_buffers);
  ...
  for (int plane = 0; plane < g_num_planes; plane++) {   // per-plane quant map (chroma subsampled -> its own layout); 4:4:4 -> all identical
    ...
    build_spatial_quant_steps(step, plane_w, plane_h, levels, quality);
    if (g_spatial_dct && (plane > 0)) {
      build_dct_quant_matrix_chroma(step, plane_w, plane_h, quality);   // dedicated chroma quant matrix (matches the CPU oracle)
    }
  ...
  if (mode_3ddwt) {
    ...
    for (;;) {
      ...
      decode_gop_3ddwt(gop_encoded, gop_encoded_length, filled, width, height, levels, quality, cpu_gop_rgb, NULL, NULL, 0, 0, 0, NULL, 0, NULL);
      ...
      for (int g = 0; g < filled; g++) {
        ...
        uint32_t mv_length;
        const uint8_t *frame_data = parse_frame_header(gop_encoded[g], SIZE_MAX, block_count_plane, &parsed_block_count, parse_offsets, &mv_data, &mv_length, NULL);
        (void)mv_data;
        (void)mv_length;
    ...
    while ((fread(rgb, 1, frame_bytes, input_pipe) == frame_bytes) && ((!max_frames) || (frame_index < max_frames))) {
      ...
      const uint8_t *frame_data = parse_frame_header(encoded, SIZE_MAX, block_count_plane, &frame_block_count, parse_offsets, &mv_data, &mv_length, &block_data_length);   // intra: no motion
      (void)mv_data;
      (void)mv_length;
      size_t data_length = block_data_length;
      ...
      // Present for both lossy (float DCT) and lossless (integer DCT) DCT-mode; absent in wavelet mode.
      if (g_spatial_dct) {
        const uint8_t *table_blobs[MAX_PLANES];
        uint32_t table_lens[MAX_PLANES];
        parse_entropy_section(frame_data + block_data_length, g_num_planes, table_blobs, table_lens);
        for (int plane = 0; plane < g_num_planes; plane++) {
      ...
      for (int plane = 0; plane < g_num_planes; plane++) {
        ...
        // Entropy decode -> integer coefficients in coeff_buffer. DCT mode (lossy + lossless integer-DCT): GPU rANS.
        if (g_spatial_dct) {
          int32_t rans_push[4] = { plane_w, plane_h, g_block_size, (int)block_data_length };
        ...
        if (g_spatial_dct && !lossless) {
          // DCT lossy: a single inverse 8x8 block DCT (float).
          ...
          // DCT lossless: reversible integer inverse 8x8 block DCT (Phase A3); no dequant/round.
          int32_t dct_push[2] = { plane_w, plane_h };
