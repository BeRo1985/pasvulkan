 ...
 *
 * Self-made wavelet audio codec, sister to the FVD video codec; used as the "FWAC" audio sub-codec inside the
 * FVD container, and standalone via the FWA_NO_MAIN-guarded main()/CLI at the bottom. Per-channel decorrelation
 ...
 * ingest/normalise the input PCM (like FVD); the codec itself is third-party-free at runtime.
 */
...
#include "fwa_audio.h"   // this file is the Flexible Wavelet Audio (FWA) codec linked into FVD as the "FWAC"
                          ...
                          // (guarded by #ifndef FWA_NO_MAIN) is excluded; fvd feeds PCM via extract_audio_pcm.
                          // fwa_encode/fwa_decode (appended below the guard) are the only non-static symbols, so
                          // every fwa internal stays file-local and nothing clashes with fvdref.c.
...
// fvd's qoal_encode/qoal_decode convention: interleaved int16 PCM, `samples` = frames per channel, and a
