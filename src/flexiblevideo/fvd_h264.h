 ...
 *                                                                            *
 ******************************************************************************/
// fvd_h264.h — H.264 hardware-decode path.
// fvdplay creates ONE shared device (with the video-decode queue + VK_KHR_video extensions) and, when
...
typedef struct {
  ...
  // audio is already decoded + queued by fvdplay (OGGV/QOAL/RPCM); the H.264 path only paces to it.
