 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.LoopOrderIndependentTransparencyResolveRenderPass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass }
     TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass=class(TpvFrameGraph.TRenderPass)
      private
...
constructor TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
...
//SeparateCommandBuffer:=true;
 ...
 if fInstance.Scene3D.EnableWater then begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.AcquirePersistentResources;
var Stream:TStream;
begin
 ...
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  if fInstance.Scene3D.EnableWater then begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_reversedz_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_frag.spv');
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
  ...
  if fInstance.Scene3D.EnableWater then begin
   if fInstance.Renderer.SupersampleWaterWhenMSAA then begin
    if fInstance.ZFar<0.0 then begin
     Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_reversedz_msaa_frag.spv');
    end else begin
     Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_msaa_frag.spv');
    end;
   end else begin
    if fInstance.ZFar<0.0 then begin
     Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_reversedz_msaa_no_msaa_water_frag.spv');
    end else begin
     Stream:=pvScene3DShaderVirtualFileSystem.GetFile('loopoit_resolve_water_msaa_no_msaa_water_frag.spv');
    end;
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.AcquireVolatileResources;
...
begin
 ...
 if fInstance.Scene3D.EnableWater then begin
  fVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,3*fInstance.Renderer.CountInFlightFrames);
 end else begin
  fVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,2*fInstance.Renderer.CountInFlightFrames);
 end;
 fVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER,2*fInstance.Renderer.CountInFlightFrames);
 fVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1*fInstance.Renderer.CountInFlightFrames);
 if fInstance.Renderer.SurfaceSampleCountFlagBits<>TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  fVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1*fInstance.Renderer.CountInFlightFrames);
 end;
  ...
  if fInstance.Scene3D.EnableWater then begin
   ...
   if fInstance.Renderer.SurfaceSampleCountFlagBits<>TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
    ...
    fVulkanDescriptorSets[InFlightFrameIndex].WriteToDescriptorSet(6,
                                                                   ...
                                                                   [],
                                                                   [fInstance.LuminanceVulkanBuffers[(InFlightFrameIndex+(FrameGraph.CountInFlightFrames-1)) mod FrameGraph.CountInFlightFrames].DescriptorBufferInfo],
                                                                   [],
                                                                   false
                                                                  );
   end;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.ReleaseVolatileResources;
var InFlightFrameIndex:TpvSizeInt;
begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyResolveRenderPass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;const aInFlightFrameIndex,aFrameIndex:TpvSizeInt);
begin
 ...
 if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
  fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Draw,'LoopOITResolve');
 end;
