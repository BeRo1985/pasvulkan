 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.RadialBlurRenderPass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesRadialBlurRenderPass }
      // Radial (zoom) blur over the finished picture: see radialblur.frag for what it does and why it sits
      // after the tone mapping rather than before it. Driven by TpvScene3DRendererInstance.RadialBlurFactor,
      // and a no-op - one plain copy - while that is zero, so it costs nothing worth measuring when the
      // game is not asking for it.
      TpvScene3DRendererPassesRadialBlurRenderPass=class(TpvFrameGraph.TRenderPass)
       private
        type TPushConstants=packed record
              CentreStrengthInner:TpvVector4; // xy = centre in [0..1], z = strength, w = inner radius
              SquaredFallOffFactor:TpvFloat;  // 0 = straight falloff, 1 = squared, anything between blends
             end;
       private
...
{ TpvScene3DRendererPassesRadialBlurRenderPass }

constructor TpvScene3DRendererPassesRadialBlurRenderPass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
...
{fResourceColor:=AddImageInput('resourcetype_color_tonemapping',
 ...
 fResourceSurface:=AddImageOutput('resourcetype_color_radialblur',
                                  'resource_radialblur_color',
...
destructor TpvScene3DRendererPassesRadialBlurRenderPass.Destroy;
begin
 inherited Destroy;
end;

procedure TpvScene3DRendererPassesRadialBlurRenderPass.AcquirePersistentResources;
var Stream:TStream;
begin
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('radialblur_frag.spv');
 try
...
procedure TpvScene3DRendererPassesRadialBlurRenderPass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesRadialBlurRenderPass.AcquireVolatileResources;
...
begin
 ...
 fVulkanPipelineLayout.AddPushConstantRange(TVkShaderStageFlags(VK_SHADER_STAGE_FRAGMENT_BIT),0,SizeOf(TpvScene3DRendererPassesRadialBlurRenderPass.TPushConstants));
...
procedure TpvScene3DRendererPassesRadialBlurRenderPass.ReleaseVolatileResources;
var InFlightFrameIndex:TpvSizeInt;
begin
...
procedure TpvScene3DRendererPassesRadialBlurRenderPass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesRadialBlurRenderPass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;const aInFlightFrameIndex,aFrameIndex:TpvSizeInt);
begin
 ...
 // was meant for. There is nothing to interpolate and nothing per-in-flight-frame to keep.
 fPushConstants.CentreStrengthInner:=TpvVector4.InlineableCreate(fInstance.RadialBlurCentre.x,
                                                                 fInstance.RadialBlurCentre.y,
                                                                 fInstance.RadialBlurFactor,
                                                                 fInstance.RadialBlurInnerRadius);
 fPushConstants.SquaredFallOffFactor:=fInstance.RadialBlurSquaredFallOffFactor;
 aCommandBuffer.CmdPushConstants(fVulkanPipelineLayout.Handle,
                                 TVkShaderStageFlags(VK_SHADER_STAGE_FRAGMENT_BIT),
                                 0,
                                 SizeOf(TpvScene3DRendererPassesRadialBlurRenderPass.TPushConstants),
                                 @fPushConstants);
 if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
  fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Draw,'RadialBlur');
 end;
