 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.PlanetWaterCausticsRenderPass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesPlanetWaterCausticsRenderPass }
     TpvScene3DRendererPassesPlanetWaterCausticsRenderPass=class(TpvFrameGraph.TRenderPass)
      private
...
constructor TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
 inherited Create(aFrameGraph);

 fInstance:=aInstance;

 Name:='PlanetWaterCausticsRenderPass';
 ...
 // The scene colour, as the attachment that is drawn into rather than as a storage image that is read,
 // added to and written back. Whichever of the two the rest of the chain is on at this point is the one
 // taken: with a multisampled surface the single sampled resource here is only the early resolved copy,
 // and the caustics put into it would never reach the picture that carries on.
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  ...
  fResourceColor:=AddImageInput('resourcetype_msaa_color_optimized_non_alpha',
...
procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.AcquirePersistentResources;
begin
...
procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.ReleasePersistentResources;
begin
 FreeAndNil(fWaterCaustics);
 inherited ReleasePersistentResources;
end;

procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.AcquireVolatileResources;
...
begin
 ...
 fWaterCaustics.AllocateResources(VulkanRenderPass,
...
procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.ReleaseVolatileResources;
begin
 fWaterCaustics.ReleaseResources;
 inherited ReleaseVolatileResources;
end;

procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesPlanetWaterCausticsRenderPass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;const aInFlightFrameIndex,aFrameIndex:TpvSizeInt);
begin
