 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Video.FlexibleVideo.Decoder;
{$i PasVulkan.inc}
{$ifndef fpc}
...
// of an FVDC container. Vulkan-compute on top of the shared CPU core in PasVulkan.Video.FlexibleVideo.
//
// Stage B (this step) is the GPU resource SETUP only: Create parses the container header + frame index, then
// builds the intra-decode compute pipelines (from the embedded FlexibleVideo*SPIRV constants), the
...
type EpvFlexibleVideoDecoder=class(EpvFlexibleVideo);

     { TpvFlexibleVideoDecoder }
     TpvFlexibleVideoDecoder=class(TpvFlexibleVideo)
      public
      ...
      private
       ...
       fHeader:TpvFlexibleVideo.THeader;
       fFrameEntries:TpvFlexibleVideo.TFrameEntries;
       ...
       fHFGain:TpvFlexibleVideo.TSynthesisGains;
       fLLGain:TpvFloat;
       fHasAlpha:boolean;     // color_flags bit2: an optional appended 8-bit alpha section per frame (intra, full-res)
       fAlphaPremultiplied:boolean; // (FVD has no premult bit; kept for the player API, always false)
       fSpatialDCT:boolean;   // color_flags bit7: spatial mode — DCT + rANS (set) vs wavelet + bit-plane (clear, FWV parity)
       fBframeDCT:boolean;    // color_flags bit6: B-frames use DCT + rANS (else wavelet + bit-plane)
       fQuadtree:boolean;     // color_flags bit3: adaptive DCT transform-size partitioning (per-frame partition section)
       fDeblock:boolean;      // color_flags bit5: in-loop deblocking filter
...
function TpvFlexibleVideoDecoder.PlaneHeight(const aPlane:TpvInt32):TpvInt32;
var Shift:TpvInt32;
begin
...
function TpvFlexibleVideoDecoder.BlockCountX(const aWidth:TpvInt32):TpvInt32;
begin
 result:=(aWidth+(fBlockSize-1)) div fBlockSize;
end;

function TpvFlexibleVideoDecoder.BlockCountY(const aHeight:TpvInt32):TpvInt32;
begin
 result:=(aHeight+(fBlockSize-1)) div fBlockSize;
end;

function TpvFlexibleVideoDecoder.MotionBlocksX(const aWidth:TpvInt32):TpvInt32;
begin
 result:=((aWidth+fMotionBlock)-1) div fMotionBlock;
end;

function TpvFlexibleVideoDecoder.MotionBlocksY(const aHeight:TpvInt32):TpvInt32;
begin
 result:=((aHeight+fMotionBlock)-1) div fMotionBlock;
end;

procedure TpvFlexibleVideoDecoder.ParseContainer;
...
begin
 ...
 for BFrameIndex:=0 to fFrameCount-1 do begin
  if TpvInt64(fFrameEntries[BFrameIndex].POC)>=fFrameCount then begin
   raise EpvFlexibleVideoDecoder.Create('Corrupt frame index (POC out of range)');
  end;
  if (fFrameEntries[BFrameIndex].Ref0<>-1) and ((fFrameEntries[BFrameIndex].Ref0<0) or (fFrameEntries[BFrameIndex].Ref0>=fFrameCount)) then begin
   raise EpvFlexibleVideoDecoder.Create('Corrupt frame index (Ref0 out of range)');
  end;
  if (fFrameEntries[BFrameIndex].Ref1<>-1) and ((fFrameEntries[BFrameIndex].Ref1<0) or (fFrameEntries[BFrameIndex].Ref1>=fFrameCount)) then begin
   raise EpvFlexibleVideoDecoder.Create('Corrupt frame index (Ref1 out of range)');
  end;
  if (TpvUInt64(fFrameEntries[BFrameIndex].Offset)>TpvUInt64(fStream.Size)) or
   ...
   raise EpvFlexibleVideoDecoder.Create('Corrupt frame index (frame past end of stream)');
  end;
  if ((fFrameEntries[BFrameIndex].Ref0<>-1) and (fFrameEntries[BFrameIndex].Ref1<>-1)) and
   ...
   raise EpvFlexibleVideoDecoder.Create('Corrupt frame index (zero-distance B references)');
  end;
 end;
 ...
 fAlphaPremultiplied:=false; // FVD has no premultiplied-alpha bit (bit3 is the quadtree flag)
 // FVD spatial-codec flags: bit3 = quadtree, bit5 = deblock, bit6 = B-frames use DCT, bit7 = DCT-vs-wavelet spatial mode.
 fQuadtree:=(fHeader.ColorFlags and 8)<>0;
 fDeblock:=(fHeader.ColorFlags and 32)<>0;
 fBframeDCT:=(fHeader.ColorFlags and 64)<>0;
 fSpatialDCT:=(fHeader.ColorFlags and 128)<>0;
...
function TpvFlexibleVideoDecoder.CreateDescriptorSetLayout(const aBufferCount:TpvInt32;const aHasImage:boolean):TpvVulkanDescriptorSetLayout;
var Index:TpvInt32;
begin
...
function TpvFlexibleVideoDecoder.CreatePipelineLayout(const aSetLayout:TpvVulkanDescriptorSetLayout;const aPushSize:TpvUInt32):TpvVulkanPipelineLayout;
begin
...
function TpvFlexibleVideoDecoder.CreateComputePipeline(const aData;const aDataSize:TVkSize;const aLayout:TpvVulkanPipelineLayout;const aBlockSizeSpec:boolean;const aMotionBlockSpec:boolean):TpvVulkanComputePipeline;
...
function TpvFlexibleVideoDecoder.CreateStorageBuffer(const aSize:TVkDeviceSize;const aDeviceLocal:boolean;const aName:TpvUTF8String):TpvVulkanBuffer;
begin
...
function TpvFlexibleVideoDecoder.AllocateSet(const aLayout:TpvVulkanDescriptorSetLayout):TpvVulkanDescriptorSet;
begin
 result:=TpvVulkanDescriptorSet.Create(fDescriptorPool,aLayout);
end;

procedure TpvFlexibleVideoDecoder.BindStorageBuffer(const aSet:TpvVulkanDescriptorSet;const aBinding:TpvUInt32;const aBuffer:TpvVulkanBuffer);
...
procedure TpvFlexibleVideoDecoder.BindStorageBufferOffset(const aSet:TpvVulkanDescriptorSet;const aBinding:TpvUInt32;const aBuffer:TpvVulkanBuffer;const aByteOffset,aRange:TVkDeviceSize);
var BufferInfo:TVkDescriptorBufferInfo;
begin
...
procedure TpvFlexibleVideoDecoder.BindStorageImage(const aSet:TpvVulkanDescriptorSet;const aBinding:TpvUInt32);
...
procedure TpvFlexibleVideoDecoder.BuildPipelines;
begin
 ...
 fPipeUnpack:=CreateComputePipeline(FlexibleVideoBitplaneUnpackSPIRVData,FlexibleVideoBitplaneUnpackSPIRVDataSize,fPLUnpack,true);
 if fSpatialDCT then begin
 ...
 fPipeIDWT97:=CreateComputePipeline(FlexibleVideoIdwt97rowSPIRVData,FlexibleVideoIdwt97rowSPIRVDataSize,fPLRow,false);
 fPipeIDWT53:=CreateComputePipeline(FlexibleVideoIdwt53rowSPIRVData,FlexibleVideoIdwt53rowSPIRVDataSize,fPLRow,false);
 fPipeRound:=CreateComputePipeline(FlexibleVideoRound97SPIRVData,FlexibleVideoRound97SPIRVDataSize,fPLRound,false);
 ...
 fPipeMotionAdd:=CreateComputePipeline(FlexibleVideoMotionAddSPIRVData,FlexibleVideoMotionAddSPIRVDataSize,fPLUnpack,false);
 // B-frames: the weighted L0/L1 blend (on the unpack layout, push 12) + the per-block-mode blend (own layout, push 20)
 if fHasBFrames then begin
  fPipeBidiBlend:=CreateComputePipeline(FlexibleVideoBidiBlendSPIRVData,FlexibleVideoBidiBlendSPIRVDataSize,fPLUnpack,false);
  if fHasPerBlockMode then begin
   fPLBlendMode:=CreatePipelineLayout(fDSL3,20);
   fPipeBlendMode:=CreateComputePipeline(FlexibleVideoBlendModeSPIRVData,FlexibleVideoBlendModeSPIRVDataSize,fPLBlendMode,false,true);
  end;
 end;
 ...
 if fMode3DDWT then begin
  ...
  fPipeTDWTInt:=CreateComputePipeline(FlexibleVideoTdwtIntSPIRVData,FlexibleVideoTdwtIntSPIRVDataSize,fPLTemporal,false);
  fPipeTDWTFloat:=CreateComputePipeline(FlexibleVideoTdwtFloatSPIRVData,FlexibleVideoTdwtFloatSPIRVDataSize,fPLTemporal,false);
 end;
 fPipeColor:=CreateComputePipeline(FlexibleVideoColorSPIRVData,FlexibleVideoColorSPIRVDataSize,fPLColor,false);
 if fHasAlpha then begin
  fPipeColorAlpha:=CreateComputePipeline(FlexibleVideoColorAlphaSPIRVData,FlexibleVideoColorAlphaSPIRVDataSize,fPLColorAlpha,false);
 end;
 fPipeColorHDR:=CreateComputePipeline(FlexibleVideoColorHdrSPIRVData,FlexibleVideoColorHdrSPIRVDataSize,fPLColorHDR,false);
 fPipeColorHDRSCRGB:=CreateComputePipeline(FlexibleVideoColorHdrScrgbSPIRVData,FlexibleVideoColorHdrScrgbSPIRVDataSize,fPLColorHDR,false);
 if fHasAlpha then begin
  fPipeColorHDRAlpha:=CreateComputePipeline(FlexibleVideoColorHdrAlphaSPIRVData,FlexibleVideoColorHdrAlphaSPIRVDataSize,fPLColorHDRAlpha,false);
  fPipeColorHDRSCRGBAlpha:=CreateComputePipeline(FlexibleVideoColorHdrScrgbAlphaSPIRVData,FlexibleVideoColorHdrScrgbAlphaSPIRVDataSize,fPLColorHDRAlpha,false);
 end;

end;

procedure TpvFlexibleVideoDecoder.BuildBuffersAndImage;
...
begin
 ...
 for Plane:=0 to fNumPlanes-1 do begin
  if fSpatialDCT then begin
   PlaneBytes:=TVkDeviceSize(PlaneWidth(Plane))*TVkDeviceSize(((PlaneHeight(Plane)+7) div 8)*8)*4;
  end else begin
 ...
 if fAQEnabled then begin
  ...
  try
   for AQCodeIndex:=0 to 255 do begin
    AQWeightPointer^[AQCodeIndex]:=TpvFlexibleVideo.AQWeightMin*Exp((TpvFloat(AQCodeIndex)/255.0)*AQLogSpan);
   end;
  finally
 ...
 if not assigned(fOutputImageMemory) then begin
  raise EpvFlexibleVideoDecoder.Create('Output image memory allocation failed');
 end;
...
begin
 ...
 if fSpatialDCT then begin // DCT rANS unpack sets: one per plane {data, offset, coeff, table}, 4 buffers each (+ ring slots for the B-frame ring)
  ...
  MaxBuffers:=MaxBuffers+(fNumPlanes*4);
 ...
 for Plane:=0 to fNumPlanes-1 do begin
  ...
  if fSpatialDCT then begin // DCT rANS unpack: {data, offset, coeff, table}
   fSetRANSUnpack[Plane]:=AllocateSet(fDSLRANS4);
   BindStorageBuffer(fSetRANSUnpack[Plane],0,fDataBuffer);
   BindStorageBuffer(fSetRANSUnpack[Plane],1,fOffsetBuffer[Plane]);
...
function TpvFlexibleVideoDecoder.ChromaShiftX:TpvInt32;
begin
...
function TpvFlexibleVideoDecoder.ChromaShiftY:TpvInt32;
begin
...
procedure TpvFlexibleVideoDecoder.RecordComputeBarrier(const aCommandBuffer:TpvVulkanCommandBuffer);
...
procedure TpvFlexibleVideoDecoder.RecordImageBarrier(const aCommandBuffer:TpvVulkanCommandBuffer;const aOldLayout,aNewLayout:TVkImageLayout;const aSrcAccess,aDstAccess:TVkAccessFlags;const aSrcStage,aDstStage:TVkPipelineStageFlags);
...
procedure TpvFlexibleVideoDecoder.RecordDispatch(const aCommandBuffer:TpvVulkanCommandBuffer;const aPipeline:TpvVulkanComputePipeline;const aLayout:TpvVulkanPipelineLayout;const aSet:TpvVulkanDescriptorSet;const aPushConstants:Pointer;const aPushSize:TpvUInt32;const aGroupsX,aGroupsY,aGroupsZ:TpvUInt32);
...
class function TpvFlexibleVideoDecoder.CdefDampingFromQuality(const aQuality:TpvInt32):TpvInt32;
begin
...
procedure TpvFlexibleVideoDecoder.SetCDEFFrame(const aCodingIndex:TpvInt32);
...
function TpvFlexibleVideoDecoder.ActiveSetCDEF(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
...
procedure TpvFlexibleVideoDecoder.RecordCDEF(const aCommandBuffer:TpvVulkanCommandBuffer;const aSet:TpvVulkanDescriptorSet;const aDst,aDst2:TpvVulkanBuffer;const aPlane:TpvInt32);
...
function TpvFlexibleVideoDecoder.ActiveSetApplyAQ(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
begin
 ...
 if (aFrameIndex<0) or (aFrameIndex>=fFrameCount) then begin
  raise EpvFlexibleVideoDecoder.Create('Frame index out of range');
 end;
 ...
 if not DecompressFrame(PpvUInt8Array(@fCompressedScratch[0]),CompressedLength,PpvUInt8Array(@fFrameScratch[0]),TpvSizeUInt(Length(fFrameScratch)),RawLength) then begin
  raise EpvFlexibleVideoDecoder.Create('Frame decompression failed');
 end;
 ...
 if not ParseFrameHeader(PpvUInt8Array(@fFrameScratch[0]),RawLength,BlockCount,Offsets,LeadingBlockCount,MVDataOffset,MVLength,BlockDataOffset) then begin
  raise EpvFlexibleVideoDecoder.Create('Corrupt frame header');
 end;
 ...
 fCurrentDataLength:=DataLength; // DCT: the rANS unpack push (recorded later) needs the block-data length
 ...
 // The alpha section sits right after the color block data, except in DCT mode where the per-plane rANS entropy
 // tables come first (block data -> entropy section -> alpha section).
 AlphaSectionOffset:=BlockDataOffset+DataLength;
 if fSpatialDCT then begin
  // DCT: parse the appended per-plane rANS frequency tables ({u32 length; exp-golomb blob}) and build the GPU table.
  EntropyOffset:=BlockDataOffset+DataLength;
  for Plane:=0 to fNumPlanes-1 do begin
   TableLength:=ReadU32LE(PpvUInt8Array(@fFrameScratch[0]),EntropyOffset);
   inc(EntropyOffset,4);
   DataPointer:=PpvUInt8Array(fRANSTableBuffer[Plane].Memory.MapMemory);
   try
    FillChar(DataPointer^[0],TpvFlexibleVideo.RANSGPUTableUInt32Count*4,#0);
    TpvFlexibleVideo.BuildRANSGPUTable(PpvUInt8Array(@fFrameScratch[EntropyOffset]),TableLength,PpvUInt32Array(@DataPointer^[0]));
   finally
    fRANSTableBuffer[Plane].Memory.UnmapMemory;
   end;
...
procedure TpvFlexibleVideoDecoder.RecordDecode(const aCommandBuffer:TpvVulkanCommandBuffer;const aIsPredicted:boolean);
...
begin
 ...
 for Plane:=0 to fNumPlanes-1 do begin
  ...
  if fSpatialDCT and fQuadtree then begin
   ...
   UnpackPush[3]:=TpvInt32(fCurrentDataLength);
   RecordDispatch(aCommandBuffer,fPipeRANSUnpack,fPLRANSUnpack,fSetRANSUnpack[Plane],@UnpackPush[0],16,PlaneBlocksX*PlaneBlocksY,1,1);
  end else begin
   UnpackPush[0]:=PlaneW;
   UnpackPush[1]:=PlaneH;
  ...
  if fSpatialDCT and fQuadtree then begin
   ...
   UnpackPush[1]:=PlaneH;
   if fLossless then begin
    RecordDispatch(aCommandBuffer,fPipeDCTInverseInteger,fPLDCTInverse,fSetRow[Plane],@UnpackPush[0],8,(PlaneW+7) div 8,(PlaneH+7) div 8,1);
   end else begin
    RecordDispatch(aCommandBuffer,fPipeDCTInverse,fPLDCTInverse,fSetRow[Plane],@UnpackPush[0],8,(PlaneW+7) div 8,(PlaneH+7) div 8,1);
   end;
   RecordComputeBarrier(aCommandBuffer);
  end else begin
  LevelCount:=0;
...
begin
 ...
 if not DecompressFrame(PpvUInt8Array(@fAlphaCompressedScratch[0]),CompressedLength,PpvUInt8Array(@fAlphaFrameScratch[0]),TpvSizeUInt(Length(fAlphaFrameScratch)),RawLength) then begin
  raise EpvFlexibleVideoDecoder.Create('Alpha frame decompression failed');
 end;
...
function TpvFlexibleVideoDecoder.ActiveDataBuffer:TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
  result:=fDataBuffer;
 end else begin
...
function TpvFlexibleVideoDecoder.ActiveOffsetBuffer(const aPlane:TpvInt32):TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveStepBuffer(const aPlane:TpvInt32):TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveTileCodesBuffer:TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveMVBuffer:TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
  result:=fMVBuffer;
 end else begin
...
function TpvFlexibleVideoDecoder.ActiveMV1Buffer:TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
  result:=fMV1Buffer;
 end else begin
...
function TpvFlexibleVideoDecoder.ActiveModeBuffer:TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
  result:=fModeBuffer;
 end else begin
...
function TpvFlexibleVideoDecoder.ActiveSetUnpack(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveSetDequant(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveRANSTableBuffer(const aPlane:TpvInt32):TpvVulkanBuffer;
begin
 if fBufferRingSlot<0 then begin
  result:=fRANSTableBuffer[aPlane];
 end else begin
  result:=fRingRANSTableBuffer[fBufferRingSlot][aPlane];
 end;
end;

function TpvFlexibleVideoDecoder.ActiveSetRANSUnpack(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
  result:=fSetRANSUnpack[aPlane];
 end else begin
  result:=fRingSetRANSUnpack[fBufferRingSlot][aPlane];
 end;
end;
...
function TpvFlexibleVideoDecoder.ActiveSetGMC1(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveSetGBlend(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveSetGBlendMode(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveSetGAdd(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
function TpvFlexibleVideoDecoder.ActiveSetMCPlay(const aPlane:TpvInt32):TpvVulkanDescriptorSet;
begin
 if fBufferRingSlot<0 then begin
...
procedure TpvFlexibleVideoDecoder.BuildBidiRing;
...
begin
 ...
 for Slot:=0 to fBufferRingSize-1 do begin
  ...
  for Plane:=0 to fNumPlanes-1 do begin
   ...
   if fSpatialDCT then begin // DCT-B rANS unpack set: {ring data, ring offset, shared coeff, ring table}
    fRingRANSTableBuffer[Slot][Plane]:=CreateStorageBuffer(TpvFlexibleVideo.RANSGPUTableUInt32Count*4,false,'FVD.ring.ranstable');
    fRingSetRANSUnpack[Slot][Plane]:=AllocateSet(fDSLRANS4);
    BindStorageBuffer(fRingSetRANSUnpack[Slot][Plane],0,fRingDataBuffer[Slot]);
    BindStorageBuffer(fRingSetRANSUnpack[Slot][Plane],1,fRingOffsetBuffer[Slot][Plane]);
...
procedure TpvFlexibleVideoDecoder.UploadBidiFrame(const aCodingIndex:TpvInt32);
var Plane,PlanePixels,StepSlot:TpvInt32;
    Entry:PFrameEntry;
    CompressedLength:TpvSizeUInt;
    RawLength,DataLength:TpvUInt32;
...
begin
 ...
 // DCT: the per-plane rANS frequency tables ({u32 length; exp-golomb blob}) sit right after the color block data.
 if fSpatialDCT then begin
  EntropyOffset:=BlockDataOffset+DataLength;
  for Plane:=0 to fNumPlanes-1 do begin
   TableLength:=ReadU32LE(PpvUInt8Array(@fFrameScratch[0]),EntropyOffset);
   inc(EntropyOffset,4);
   DataPointer:=PpvUInt8Array(ActiveRANSTableBuffer(Plane).Memory.MapMemory);
   try
    FillChar(DataPointer^[0],TpvFlexibleVideo.RANSGPUTableUInt32Count*4,#0);
    TpvFlexibleVideo.BuildRANSGPUTable(PpvUInt8Array(@fFrameScratch[EntropyOffset]),TableLength,PpvUInt32Array(@DataPointer^[0]));
   finally
...
procedure TpvFlexibleVideoDecoder.RecordBidiDecode(const aCommandBuffer:TpvVulkanCommandBuffer;const aIsPredicted,aRef1Slot,aWeight0,aWeight1:TpvInt32);
var Plane,Level,LevelCount:TpvInt32;
    PlaneW,PlaneH,ScratchStride,PlanePixels:TpvInt32;
    PlaneBlocksX,PlaneBlocksY,PlaneBlockCount:TpvInt32;
    PlanePixelWorkgroups,PlaneUnpackWorkgroups,PlaneMotionBlocksX:TpvInt32;
    DequantPixels,DequantWorkgroups:TpvInt32;
    CurrentWidth,CurrentHeight,LevelW,LevelH:TpvInt32;
    LevelWidth,LevelHeight:array[0..15] of TpvInt32;
    RowPipeline:TpvVulkanComputePipeline;
    UnpackPush:array[0..3] of TpvInt32;
...
begin
 ...
 for Plane:=0 to fNumPlanes-1 do begin
  ...
  if fSpatialDCT and fQuadtree then begin
   ...
   UnpackPush[2]:=fBlockSize;
   UnpackPush[3]:=TpvInt32(fCurrentDataLength);
   RecordDispatch(aCommandBuffer,fPipeRANSUnpack,fPLRANSUnpack,ActiveSetRANSUnpack(Plane),@UnpackPush[0],16,PlaneBlocksX*PlaneBlocksY,1,1);
  end else begin
   UnpackPush[0]:=PlaneW;
   UnpackPush[1]:=PlaneH;
   UnpackPush[2]:=PlaneBlocksX;
  ...
  if not fLossless then begin
   ...
   DequantWorkgroups:=(DequantPixels+255) div 256;
   DequantPush[0]:=DequantPixels;
   if Plane=0 then begin
    ChromaMultiplier:=1.0;
   end else begin
    ChromaMultiplier:=fChromaQuant;
   ...
   RecordDispatch(aCommandBuffer,fPipeDequant,fPLDequant,ActiveSetDequant(Plane),@DequantPush[0],8,DequantWorkgroups,1,1);
   RecordComputeBarrier(aCommandBuffer);
  end;
...
function TpvFlexibleVideoDecoder.PrepareBidiFrame(const aDisplayPOC:TpvInt32;out aIsPredicted,aRef1Slot,aWeight0,aWeight1:TpvInt32):boolean;
...
begin
 ...
 if DstSlot<0 then begin
  ...
  raise EpvFlexibleVideoDecoder.Create('B-frame DPB pool exhausted');
 end;
...
procedure TpvFlexibleVideoDecoder.RecordBidiDisplay(const aCommandBuffer:TpvVulkanCommandBuffer;const aDisplayPOC:TpvInt32);
...
procedure TpvFlexibleVideoDecoder.DecodeFrameBidi(const aCommandBuffer:TpvVulkanCommandBuffer;const aDisplayPOC:TpvInt32);
 ...
 while fGCursor<fFrameCount do begin
  ...
  if fSubmitMode=1 then begin // mode B: this coding frame uses the next per-frame ring slot
   if RingIndex>=fBufferRingSize then begin
    raise EpvFlexibleVideoDecoder.Create('B-frame mode B input ring overflow');
   end;
...
function TpvFlexibleVideoDecoder.DecodeFrameStep(const aCommandBuffer:TpvVulkanCommandBuffer;const aDisplayPOC:TpvInt32):boolean;
...
function TpvFlexibleVideoDecoder.GopCountFrom(const aStart:TpvInt32):TpvInt32;
begin
...
procedure TpvFlexibleVideoDecoder.Upload3DFrame(const aCodingIndex,aSlot,aGOPCount:TpvInt32);
...
procedure TpvFlexibleVideoDecoder.RecordSpatial3D(const aCommandBuffer:TpvVulkanCommandBuffer;const aBuf,aSlot:TpvInt32);
var Plane,Level,LevelCount:TpvInt32;
    PlaneW,PlaneH,ScratchStride,PlanePixels:TpvInt32;
    PlaneBlocksX,PlaneBlocksY,PlaneBlockCount:TpvInt32;
    PlanePixelWorkgroups,PlaneUnpackWorkgroups:TpvInt32;
...
procedure TpvFlexibleVideoDecoder.RecordDisplay3D(const aCommandBuffer:TpvVulkanCommandBuffer;const aBuf,aSlot:TpvInt32);
...
procedure TpvFlexibleVideoDecoder.PrefetchWait;
begin
...
procedure TpvFlexibleVideoDecoder.PrefetchFinishGop(const aBuf,aGOPCount:TpvInt32);
begin
...
procedure TpvFlexibleVideoDecoder.PrepareFrame3D(const aDisplayIndex:TpvInt32);
...
procedure TpvFlexibleVideoDecoder.DecodeFrame3D(const aCommandBuffer:TpvVulkanCommandBuffer;const aDisplayIndex:TpvInt32);
begin
...
procedure TpvFlexibleVideoDecoder.DecodeFrame(const aCommandBuffer:TpvVulkanCommandBuffer;const aFrameIndex:TpvInt32);
begin
 if fMode3DDWT then begin
...
procedure TpvFlexibleVideoDecoder.PrepareFrameBidi(const aDisplayPOC:TpvInt32);
var IsPredicted,Ref1Slot,Weight0,Weight1,RingIndex:TpvInt32;
begin
...
procedure TpvFlexibleVideoDecoder.RecordFrameBidi(const aCommandBuffer:TpvVulkanCommandBuffer);
...
procedure TpvFlexibleVideoDecoder.PrepareFrame(const aDisplayIndex:TpvInt32);
begin
...
procedure TpvFlexibleVideoDecoder.RecordFrame(const aCommandBuffer:TpvVulkanCommandBuffer);
begin
...
procedure TpvFlexibleVideoDecoder.ResetForReplay;
var Index:TpvInt32;
begin
