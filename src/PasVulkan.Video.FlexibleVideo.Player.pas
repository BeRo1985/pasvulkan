unit PasVulkan.Video.FlexibleVideo.Player;
{$i PasVulkan.inc}
...
type EpvFlexibleVideoPlayer=class(EpvFlexibleVideo);

     { TpvFlexibleVideoPlayer }
     TpvFlexibleVideoPlayer=class(TObject)
      public
       type TDecoderChoice=
             (
      ...
      private
       ...
       fDecoder:TpvFlexibleVideoDecoder; // the wavelet backend (nil when the H.264 backend is active)
...
begin
 ...
 if (not CompareMem(@fHeader.Magic[0],@TpvFlexibleVideo.Magic[0],4)) or (fHeader.Version<>TpvFlexibleVideo.FormatVersion) then begin
  raise EpvFlexibleVideoPlayer.Create('Not a FVDC stream');
 end;
...
{$ifdef VkVideo}
 ...
 if (not fUsingH264) and (aDecoderChoice=TDecoderChoice.ForceH264) then begin
  raise EpvFlexibleVideoPlayer.Create('H.264 hardware decode requested but unavailable (no stream, no VK video-decode queue, or session creation failed)');
 end;
{$else}
 if aDecoderChoice=TDecoderChoice.ForceH264 then begin
  raise EpvFlexibleVideoPlayer.Create('H.264 hardware decode requested but the engine was built without VkVideo');
 end;
{$endif}
...
function TpvFlexibleVideoPlayer.GetAudioFinished:boolean;
begin
...
function TpvFlexibleVideoPlayer.GetPlaybackFinished:boolean;
begin
...
function TpvFlexibleVideoPlayer.AudioReadCallback(const aFloatBuffer:Pointer;const aFrameCount:TpvInt32):TpvInt32;
...
procedure TpvFlexibleVideoPlayer.OpenAudio(const aAudio:TpvAudio);
...
procedure TpvFlexibleVideoPlayer.StartAudio;
begin
...
procedure TpvFlexibleVideoPlayer.PauseAudio;
begin
 if assigned(fAudioStream) then begin
  fAudioStream.Pause;
 end;
...
procedure TpvFlexibleVideoPlayer.ResumeAudio;
begin
 if assigned(fAudioStream) then begin
  fAudioStream.Resume;
 end;
...
procedure TpvFlexibleVideoPlayer.SeekAudio(const aTimeInSeconds:TpvDouble);
var Frame:TpvInt64;
begin
...
procedure TpvFlexibleVideoPlayer.CloseAudio;
begin
...
function TpvFlexibleVideoPlayer.MasterClockSeconds:TpvDouble;
begin
...
function TpvFlexibleVideoPlayer.GetOutputImage:TpvVulkanImage;
begin
...
function TpvFlexibleVideoPlayer.GetOutputImageView:TpvVulkanImageView;
begin
...
function TpvFlexibleVideoPlayer.GetOutputFormat:TVkFormat;
begin
 if fUsingH264 then begin
...
function TpvFlexibleVideoPlayer.GetIsHDR:boolean;
begin
...
function TpvFlexibleVideoPlayer.GetHasAlpha:boolean;
begin
 if fUsingH264 then begin
...
function TpvFlexibleVideoPlayer.GetAlphaPremultiplied:boolean;
begin
 if fUsingH264 then begin
  result:=false;
  exit;
...
function TpvFlexibleVideoPlayer.TimeToFrameIndex(const aTimeInSeconds:TpvDouble):TpvInt32;
...
function TpvFlexibleVideoPlayer.DecodeTime(const aTimeInSeconds:TpvDouble):boolean;
begin
...
function TpvFlexibleVideoPlayer.Decode(const aCommandBuffer:TpvVulkanCommandBuffer):boolean;
begin
...
procedure TpvFlexibleVideoPlayer.Restart;
begin
