...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fParticleVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fParticleVertexShaderModule');
 ...
 if fInstance.Renderer.Scene3D.RaytracingActive then begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_'+OITVariant+'lockoit_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_msaa_'+OITVariant+'lockoit_frag.spv');
  end;
 end else begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_'+OITVariant+'lockoit_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_msaa_'+OITVariant+'lockoit_frag.spv');
  end;
 end;
 try
...
begin
 ...
 for InFlightFrameIndex:=0 to FrameGraph.CountInFlightFrames-1 do begin
  ...
  try
   ...
   DescriptorImageInfos[1]:=TVkDescriptorImageInfo.Create(fInstance.Renderer.ClampedSampler.Handle,
                                                          fInstance.SceneMipmappedArray2DImage.VulkanArrayImageView.Handle,
                                                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
   DescriptorImageInfos[2]:=TVkDescriptorImageInfo.Create(fInstance.Renderer.ClampedNearestSampler.Handle,
