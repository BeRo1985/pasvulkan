...
begin
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('global_illumination_cascaded_radiance_hints_inject_sky_comp.spv');
 try
...
begin
 ...
 BufferMemoryBarrier:=TVkBufferMemoryBarrier.Create(TVkAccessFlags(VK_ACCESS_HOST_WRITE_BIT) or TVkAccessFlags(VK_ACCESS_TRANSFER_WRITE_BIT),
                                                    ...
                                                    VK_QUEUE_FAMILY_IGNORED,
                                                    fInstance.GlobalIlluminationRadianceHintsUniformBuffers[InFlightFrameIndex].Handle,
                                                    0,
                                                    VK_WHOLE_SIZE);
 ...
 if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
  fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Dispatch,'GIRadianceHintsInjectSky');
 end;
