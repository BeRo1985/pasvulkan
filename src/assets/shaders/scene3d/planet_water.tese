...
void main(){	  
  ...
  // Wave height displacement (disabled when waveDisplaceAmplitude == 0 in planet data).
  {
    vec4 uvwp1 = vec4(unpackHalf2x16(planetData.waterUVWaveParams.z), unpackHalf2x16(planetData.waterUVWaveParams.w));
    float displaceAmpl = uvwp1.w;
    if(displaceAmpl > 0.0){
      vec4 uvwp0 = vec4(unpackHalf2x16(planetData.waterUVWaveParams.x), unpackHalf2x16(planetData.waterUVWaveParams.y));
      float waveH = computeWaveDisplacement(octPlanetUnsignedEncode(sphereNormal), pushConstants.time, uvwp0.y, uvwp0.z, uvwp1.z) * displaceAmpl;
