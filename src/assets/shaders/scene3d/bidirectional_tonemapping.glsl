...
#define BIDIRECTIONAL_TONEMAPPING_VARIANT_CHANNELWISE 6 // Channel-wise tonemapping operator

// AMD has black-artefacts at too bright values at MSAA resolving, so it's not used here.

#define BIDIRECTIONAL_TONEMAPPING_VARIANT BIDIRECTIONAL_TONEMAPPING_VARIANT_CHANNELWISE
...
#elif BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_CHANNELWISE
  // Channel-wise tonemapping operator
  return color / (abs(color) + vec3(1.0));
...
#elif BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_CHANNELWISE
  // Channel-wise tonemapping operator
  return color / max(vec3(1.0) - abs(color), vec3(1e-5));
#else // BIDIRECTIONAL_TONEMAPPING_VARIANT_NONE
