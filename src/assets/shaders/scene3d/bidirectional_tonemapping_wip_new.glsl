...
// Luminance scale factor for the luminance-based reversible tonemapping operators (Brian Karis and AMD). It is passed as a
// parameter to the operator functions, so different consumers can pick a different scale without affecting each other:
//
//   - BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE (= 1.0) is the default for the plain no-argument functions and maps the
//     luminance into [0.0, 1.0). This is what forward-only / perceptual consumers (SMAA edge detection, sRGB encoding,
//     luma-threshold based antialiasing, image-to-buffer packing, ...) need, since they expect an LDR-like [0.0, 1.0]
//     result and never invert it.
//
//   - BIDIRECTIONAL_TONEMAPPING_RESOLVE_LUMA_SCALE (= 1.0 / 32.0) is meant for reversible resolve / blend / upsample
//     round-trips (forward -> filter -> inverse). At scale 1.0 every bright value (highlights, bloom, emissive) collapses
//     against the saturation point at luminance 1.0, where the inverse 1.0 / (1.0 - luma) becomes singular: tiny blend /
//     clip differences near the saturation point explode into fireflies/sparkles, and if the tonemapped value additionally
//     transits a half-float (R16G16B16A16_SFLOAT) intermediate buffer between two passes (e.g. EASU -> RCAS) every
//     luminance above roughly 2000-5000 rounds to exactly 1.0 and is unrecoverable. The smaller scale moves the saturation
//     point far out (the tonemapped range becomes [0.0, 1.0 / scale)), so the whole fp16 range reconstructs faithfully
//     while the operator still compresses HDR firefly outliers massively (e.g. 65504 -> ~32 at scale 1.0 / 32.0).
//
// Both ends of a reversible round-trip MUST use the exact same scale to stay exactly reversible:
//   t = c / ((scale * luma(c)) + 1.0) and c = t / (1.0 - (scale * luma(t))).
const float BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE = 1.0;
const float BIDIRECTIONAL_TONEMAPPING_RESOLVE_LUMA_SCALE = 1.0 / 32.0;

vec3 ApplyToneMapping(vec3 color, float lumaScale){
  ...
  return color / ((lumaScale * max(max(color.x, color.y), color.z)) + 1.0);
#elif BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_BRIAN_KARIS
  // http://graphicrants.blogspot.com/2013/12/tone-mapping.html
  return color / ((lumaScale * abs(dot(color, vec3(0.2125, 0.7154, 0.0721)))) + 1.0); // abs is used to avoid negative values, since the dot product can be negative for scRGB values (extended sRGB for HDR). The luminance scale moves the saturation point far out (see BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE / BIDIRECTIONAL_TONEMAPPING_RESOLVE_LUMA_SCALE).
...
vec3 ApplyInverseToneMapping(vec3 color, float lumaScale){
#if BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_ACES
...
#elif BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_AMD
  // https://gpuopen.com/learn/optimized-reversible-tonemapper-for-resolve/ 
  return color / max(1.0 - (lumaScale * max(max(color.x, color.y), color.z)), 1e-5);
#elif BIDIRECTIONAL_TONEMAPPING_VARIANT == BIDIRECTIONAL_TONEMAPPING_VARIANT_BRIAN_KARIS
  // http://graphicrants.blogspot.com/2013/12/tone-mapping.html
  return color / max(1.0 - (lumaScale * abs(dot(color, vec3(0.2125, 0.7154, 0.0721)))), 1e-5); // Even here, abs is used to avoid negative values, since the dot product can be negative for scRGB values (extended sRGB for HDR). The luminance scale must match the one in the forward ApplyToneMapping to stay exactly reversible. 
...
vec4 ApplyToneMapping(vec4 color, float lumaScale){
  return vec4(vec3(ApplyToneMapping(color.xyz, lumaScale)), color.w);
}

vec4 ApplyInverseToneMapping(vec4 color, float lumaScale){
  return vec4(vec3(ApplyInverseToneMapping(color.xyz, lumaScale)), color.w);
}

// Default no-argument variants use BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE (= 1.0), i.e. the bounded [0.0, 1.0) mapping that
// forward-only / perceptual consumers expect. Reversible resolve / blend / upsample passes should call the explicit
// lumaScale overloads with BIDIRECTIONAL_TONEMAPPING_RESOLVE_LUMA_SCALE instead (the same scale on both ends).
vec3 ApplyToneMapping(vec3 color){
  return ApplyToneMapping(color, BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE);
}

vec3 ApplyInverseToneMapping(vec3 color){
  return ApplyInverseToneMapping(color, BIDIRECTIONAL_TONEMAPPING_LUMA_SCALE);
}
