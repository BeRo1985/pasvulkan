          ...
          if (lightTreeNode.aabbMaxUserData.w != 0xffffffffu) {
            ...
            if (/*(uShadows != 0) &&*/ ((light.metaData.y & 0x80000000u) == 0u) && (uCascadedShadowMaps.metaData.x != SHADOWMAP_MODE_NONE)) {
              switch (light.metaData.x) {
...
#if defined(RAYTRACING)
                case 1u: { // Directional 
                  lightAttenuation *= getRaytracedHardShadow(rayOrigin, normalize(-light.directionZFar.xyz), rayOffset, 10000000.0);
                  break;
                }
                case 2u: {  // Point
                ...
                case 3u: {  // Spot
                  lightAttenuation *= getRaytracedHardShadow(rayOrigin, normalizedLightVector, rayOffset, 10000000.0);
                  break;
                }
#elif 0
                case 1u: { // Directional 
