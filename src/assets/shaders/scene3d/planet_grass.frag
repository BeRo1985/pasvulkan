...
#include "roughness.glsl"
  ...
  // Cascaded radiance hints SH volume at the same dedicated set 4 (mirrors planet_renderpass.frag / mesh.frag's CRH descriptor).
...
#undef LIGHTING_IMPLEMENTATION
...
#if defined(GLOBAL_ILLUMINATION_DUGI)
  // RT GI: probe-field diffuse; the probe glossy is blended into the (otherwise off) env specular source below, and the
  // env-IBL term is occluded by the probe sky-visibility (giIBLWeight, sampled along the reflection vector). Mirrors mesh.frag.
  ...
  // env-IBL block below (its source is gated by giResidualIBLSpecularWeight).
  ...
  // Cascaded voxel cone tracing: cone-traced indirect diffuse + specular (metals demoted on the diffuse, metal F0 on the
  // specular). The env IBL is off for this mode (residual weights 0, like mesh.frag), so the cones carry the indirect.
  float giIBLWeight = 1.0;
  if(dot(baseColor.xyz, vec3(1.0)) > 1e-6){
  ...
  float giIBLWeight = 1.0;
#endif
  // Environment IBL sources, each gated by its split residual weight (mesh.frag model).
