...
void main() {
  ...
  vec4 f = vec4(0.2126, 0.7152, 0.0722, 0.0),
      ...
      t = vec4(dot(textureLod(uTexture, vec3(inTexCoord + (d.yx * w), float(gl_ViewIndex)), 0.0), f),  //
               dot(textureLod(uTexture, vec3(inTexCoord + (d.xy * w), float(gl_ViewIndex)), 0.0), f),  //
               dot(textureLod(uTexture, vec3(inTexCoord + (d.zy * w), float(gl_ViewIndex)), 0.0), f),  //
               dot(textureLod(uTexture, vec3(inTexCoord + (d.yz * w), float(gl_ViewIndex)), 0.0), f));
  ...
  if (nl >= 0.0625) {
    ...
    outColor = (outColor +                                                                             //
                ((textureLod(uTexture, vec3(inTexCoord + (n * 0.5), float(gl_ViewIndex)), 0.0) * 0.9) +  //
                 (textureLod(uTexture, vec3(inTexCoord - (n * 0.5), float(gl_ViewIndex)), 0.0) * 0.9) +  //
                 (textureLod(uTexture, vec3(inTexCoord + n, float(gl_ViewIndex)), 0.0) * 0.75) +         //
                 (textureLod(uTexture, vec3(inTexCoord - n, float(gl_ViewIndex)), 0.0) * 0.75))          //
                ) /
               4.3;  //
  }
