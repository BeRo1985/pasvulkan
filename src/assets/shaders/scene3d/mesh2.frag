...
float Shadow2DPCFMultipleTapPCFContactHardend(const in sampler2DArray pTexShadowMapArrayCompare, 
  ...
  vec4 shadowNDC = shadowClipSpace / shadowClipSpace.w;
  ...
  const vec3 texelSize = vec3(vec2(1.0) / textureSize(pTexShadowMapArrayCompare, 0).xy, 0);
  const float penumbra = 4.0 / textureSize(pTexShadowMapArrayCompare, 0).x;
  float lightDistanceNormalized = shadowNDC.z;
#if 1
  for(int tapIndex = 0; tapIndex < SHADOW_TAP_COUNT; tapIndex++){
    vec2 offset = Taps2D[tapIndex] * rotationMatrix * penumbra;
    ...
    if (all(greaterThanEqual(uv, vec2(0.0))) && all(lessThanEqual(uv, vec2(1.0)))) {
      ...
      float sampleDistance = sampleDepth - lightDistanceNormalized;
  ...
  return ContactHardenPCFKernel(occluders, occluderDistSum, lightDistanceNormalized, 1.0);
#else  
/*
...
*/
  for(int tapIndex = 0; tapIndex < SHADOW_TAP_COUNT; tapIndex++){
    vec2 offset = Taps2D[tapIndex] * rotationMatrix * penumbra;
    vec4 samples = textureGather(pTexShadowMapArrayCompare, vec3(shadowNDC.xy + offset, float(cascadedShadowMapIndex)));
    vec4 sampleDistances = samples - vec4(lightDistanceNormalized);
  ...
  return ContactHardenPCFKernel(occluders, occluderDistSum, lightDistanceNormalized, 4.0);
#endif
}  

float doCascadedShadowMapShadow(const in int cascadedShadowMapIndex, const in vec3 lightDirection) {
#if 1
  return 1.0 - Shadow2DPCFMultipleTapPCFContactHardend(uCascadedShadowMapTexture, cascadedShadowMapIndex);
#else  
  mat4 shadowMapMatrix = uCascadedShadowMaps.shadowMapMatrices[cascadedShadowMapIndex];
  vec4 shadowClipSpace = shadowMapMatrix * vec4(inWorldSpacePosition, 1.0);
